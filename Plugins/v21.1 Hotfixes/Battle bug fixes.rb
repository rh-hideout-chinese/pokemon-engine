  ...
  def pbStorePokemon(pkmn)
    ...
    if !pkmn.shadowPokemon?
      if $PokemonSystem.givenicknames == 0 &&
         pbDisplayConfirm(_INTL("你想给{1}起个名字吗?", pkmn.name))
    ...
    if pbPlayer.party_full? && (@sendToBoxes == 0 || @sendToBoxes == 2)   # Ask/must add to party
      cmds = [_INTL("加入队伍"),
              _INTL("寄去电脑盒子"),
              _INTL("查看{1}的摘要", pkmn.name),
              _INTL("检查队伍")]
      cmds.delete_at(1) if @sendToBoxes == 2   # Remove "Send to a Box" option
      loop do
        cmd = pbShowCommands(_INTL("你想把{1}送到哪里?", pkmn.name), cmds, 99)
        ...
        when 0   # Add to your party
          pbDisplay(_INTL("选择你队伍中的一只宝可梦，将其送入你的盒子。"))
          party_index = -1
          ...
          pbDisplayPaused(_INTL("{1}传送到了盒子\"{2}\".", send_pkmn.name, box_name))
    ...
    if stored_box < 0
      pbDisplayPaused(_INTL("{1}已经被加入到了队伍", pkmn.name))
      @initialItems[0][pbPlayer.party.length - 1] = pkmn.item_id if @initialItems
      return
    end
    # Messages saying the Pokémon was stored in a PC box
    box_name = @peer.pbBoxName(stored_box)
    pbDisplayPaused(_INTL("{1}已经传送到盒子了\"{2}\"!", pkmn.name, box_name))
  end
end
