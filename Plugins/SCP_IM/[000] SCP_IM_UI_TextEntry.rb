  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    if USEKEYBOARD
      @sprites["entry"] = Window_TextEntry_Keyboard.new(
		 initialText, 0, 12, 400 - 112, 96, helptext, true
	  )
      Input.text_input = true
    else
      @sprites["entry"]=Window_TextEntry.new(
		 initialText, 0, 0, 400, 96, helptext, true
	  )
    end
    @sprites["entry"].x = (Graphics.width / 2) - (@sprites["entry"].width / 2) + 64
    ...
    when 4   # Storage box
      ...
      @sprites["subject"].anim_duration=4
      charwidth=@sprites["subject"].bitmap.width
      charheight=@sprites["subject"].bitmap.height
      @sprites["subject"].x = 44*2 - charwidth/8
...
class PokemonEntryScene2
  @@Characters = [
    [("             " + "QWERTYUIOP[]\\" + "ASDFGHJKL;':\"" + "ZXCVBNM,./<>?" + "             ").scan(/./), _INTL("拼音")],
    [("ABCDEFGHIJKLM" + "NOPQRSTUVWXYZ" + "abcdefghijklm" + "nopqrstuvwxyz" + "0123456789   ").scan(/./),_INTL("英文")],
    [(",.'\":;!?   ♂♀" + "~@#*&$§()    " + "【】[]{}<>《》   " + "+-×÷=±%      " + "^_/\\|        ").scan(/./),_INTL("符号")]
  ]
  ROWS    = 13
  COLUMNS = 5
  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    for i in 0...@@Characters.length
      ...
      for y in 0...COLUMNS
        for x in 0...ROWS
          pos = y * ROWS + x
          textPos.push([@@Characters[i][0][pos], 44 + x * 32, 28 + y * 38, 2,
    ...
    underline_bitmap.fill_rect(2, 2, 22, 4, Color.new(00, 102, 170))
    underline_bitmap.fill_rect(0, 0, 22, 4, Color.new(00, 187, 255))
  ...
  def pbDoUpdateOverlay2
    ...
    overlay.clear
    modeIcon = [[_INTL("Graphics/UI/Naming/icon_mode"), 48 + @mode * 64, 120, @mode * 60, 0, 60, 44]]
  ...
  def pbDoUpdateOverlay
    ...
    textPositions = [
       [@helptext, 160, 18, false, Color.new(248, 248, 248), Color.new(120, 120, 120)]
    ]
    chars = @helper.textChars #QAQ + @IM.value.split(//)
    x = 166 - 8
    for ch in chars
      textPositions.push([ch, x, 52, false, Color.new(248, 248, 248), Color.new(120, 120, 120)])
      x += 24
    end
    ...
    if !@IM.value.empty?
      ...
      x = 166
      for ch in @IM_chars
        textPositions.push([ch, x - 111, 188, false, Color.new(248, 248, 248), Color.new(120, 120, 120)])
        x += 32
      end
      pbDrawTextPositions(bgoverlay, textPositions)
    end
