  ...
  if itm.field_use && itm.is_tera_shard?
    ...
    if $bag.has?(item, qty)
      ...
      $player.party.each do |pkmn|
        ...
        annot.push((elig) ? _INTL("可以") : _INTL("不行"))
      end
      pbFadeOutIn {
        scene = PokemonParty_Scene.new
        screen = PokemonPartyScreen.new(scene, $player.party)
        screen.pbStartScene(_INTL("要使用在哪只宝可梦身上？"), false, annot)
        loop do
          scene.pbSetHelpText(_INTL("要使用在哪只宝可梦身上？"))
          ...
          if qty == 1
            pbMessage(_INTL("你用了最后一块{1}.", itm.portion_name)) { screen.pbUpdate }
          else
            pbMessage(_INTL("{1}不足……", itm.portion_name_plural)) { screen.pbUpdate }
          end
          break
        end
        screen.pbEndScene
  ...
  if itm.is_tera_shard?
    ...
    if $bag.has?(item, qty)  
      ...
      if ret
        ...
        if !$bag.has?(item, qty)
          if qty == 1
            pbMessage(_INTL("你用了最后一块{1}.", itm.name)) { scene.pbUpdate }
          else
            pbMessage(_INTL("{1}不足……", itm.portion_name_plural)) { scene.pbUpdate }
          end
        end
      end
      return ret
...
if PluginManager.installed?("Bag Screen w/int. Party")
    ...
    def pbUpdateAnnotation
      ...
      if item_data && item_data.is_tera_shard? && @bag.last_viewed_pocket == 1
        $player.party.each_with_index do |pkmn, i|
          ...
          annotation = (elig) ? _INTL("可以") : _INTL("不行")
  ...
  def pbBagUseItem(bag, item, scene, screen, chosen, bagscene=nil)
    ...
    if itm.is_machine?    # TM, HM or TR
      if $player.pokemon_count == 0
        pbMessage(_INTL("没有宝可梦")) { screen.pbUpdate }
        return 0
      end
      machine = itm.move
      return 0 if !machine
      ...
      if pkmn.egg?
        pbMessage(_INTL("蛋无法学习任何招式。")) { screen.pbUpdate }
      elsif pkmn.shadowPokemon?
        pbMessage(_INTL("暗影宝可梦无法学习任何招式。")) { screen.pbUpdate }
      elsif movelist && !movelist.any? { |j| j == pkmn.species }
        pbMessage(_INTL("{1}无法学习{2}.", pkmn.name, movename)) { screen.pbUpdate }
      ...
      if itm.is_tera_shard?
        ...
        if !$bag.has?(item, qty)
          pbMessage(_INTL("你没有足够的{1}……" +
                          "\n更改宝可梦的太晶属性需要{2}块\n太晶碎片。", itm.portion_name_plural, qty))
          return 0
        end
      end
      if pbCheckUseOnPokemon(item, pkmn, screen)
        ...
        if !$bag.has?(item)
          if itm.is_tera_shard? && qty > 1
            screen.pbDisplay(_INTL("{1}不足……", itm.portion_name_plural)) { screen.pbUpdate }
          else
            screen.pbDisplay(_INTL("你用了最后一块{1}。", itm.portion_name)) { screen.pbUpdate }
          end
      ...
      pbMessage(_INTL("这里无法使用该道具。")) { screen.pbUpdate }
      return 0
    end
  end
end
...
ItemHandlers::UseOnPokemon.addIf(:tera_shards,
  ...
  proc { |item, qty, pkmn, scene|
    ...
    if type && pkmn.tera_type != type && pkmn.getTeraType(true).nil? && !pkmn.shadowPokemon?
      ...
      when :Choose
        scene.pbDisplay(_INTL("选择{1}的新太晶属性。", pkmn.name))
    ...
    if pkmn.tera_type != old_tera
      scene.pbDisplay(_INTL("{1}的太晶属性变成了{2}。", pkmn.name, GameData::Type.get(pkmn.tera_type).name))
...
ItemHandlers::CanUseInBattle.add(:RADIANTTERAJEWEL, proc { |item, pokemon, battler, move, firstAction, battle, scene, showMessages|
  ...
  if !battle.pbHasTeraOrb?(battler.index)
    scene.pbDisplay(_INTL("你没有{1}需要蓄能！", orb)) if showMessages
    next false
  elsif !firstAction
    scene.pbDisplay(_INTL("你不能在下指令的同时使用\n这个道具！!")) if showMessages
    next false
  elsif battle.terastallize[side][owner] == -1 || (side == 0 && owner == 0 && $player.tera_charged?)
    scene.pbDisplay(_INTL("现在还不需要为{1}蓄能！", orb)) if showMessages
    next false
  end
  next true
})
...
ItemHandlers::UseInBattle.add(:RADIANTTERAJEWEL, proc { |item, battler, battle|
  ...
  battle.pbDisplayPaused(_INTL("{1}为{2}的{3}蓄能了!\n{2}现在可以使用太晶化了", item, trainer, orb))
})