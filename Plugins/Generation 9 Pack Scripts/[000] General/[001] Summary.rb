...
class PokemonSummary_Scene
  ...
  def pbOptions
    ...
    when 4
      ...
      commands[cmdTeachTMs   = commands.length] = _INTL("使用TM")
    else
      if !@pokemon.egg?
        commands[cmdGiveItem = commands.length] = _INTL("给予物品")
        commands[cmdTakeItem = commands.length] = _INTL("取回物品") if @pokemon.hasItem?
        commands[cmdNickname = commands.length] = _INTL("重新命名") if !@pokemon.foreign?
      end
    ...
    if cmdGiveItem >= 0 && command == cmdGiveItem
      ...
      nickname = pbEnterPokemonName(_INTL("{1}这个名字?", @pokemon.name), 0, Pokemon::MAX_NAME_SIZE, "", @pokemon, true)
