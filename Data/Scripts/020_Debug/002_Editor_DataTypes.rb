...
module BooleanProperty2
  def self.set(_settingname, _oldsetting)
    ret = pbShowCommands(nil, [_INTL("真"), _INTL("假")], -1)
  ...
  def self.format(value)
    return _INTL("真") if value
    return (value.nil?) ? "-" : _INTL("假")
  end
end
...
class EnumProperty
  ...
  def set(settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("为{1}选择值。", settingname), commands, -1)
...
class EnumProperty2
  ...
  def set(settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("为{1}选择值。", settingname), commands, -1, nil, oldsetting)
    return oldsetting if cmd < 0
    return cmd
  end
    ...
    loop do
      ...
      if cmd >= 0   # Chose a value
        ...
        if entry[1] == -1   # Add new value
          new_value = pbMessageFreeText(_INTL("请输入新值。"),
        ...
        case pbMessage(_INTL("保存修改？"), [_INTL("是"), _INTL("否"), _INTL("取消")], 3)
        when 0
...
class GameDataProperty
  def initialize(value)
    raise _INTL("在 module GameData 中找不到class {1}。", value.to_s) if !GameData.const_defined?(value.to_sym)
...
module TypesProperty
  def self.set(_settingname, oldsetting)
    ...
    loop do
      cmds = []
      2.times { |i| cmds.push(_INTL("属性{1}：{2}", i, ret[i] || "-")) }
      index = pbMessage(_INTL("设置此物种的属性。"), cmds, -1)
      break if index < 0
    ...
    return ret if ret != oldsetting.compact && pbConfirmMessage(_INTL("应用修改？"))
    return oldsetting
  end
...
module GenderProperty
  def self.set(_settingname, _oldsetting)
    ret = pbShowCommands(nil, [_INTL("雄性"), _INTL("雌性")], -1)
    return (ret >= 0) ? ret : nil
  end
  ...
  def self.format(value)
    return "-" if !value
    return _INTL("雄性") if value == 0
    return _INTL("雌性") if value == 1
    return "-"
  end
end
  ...
  def set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      ...
      properties[s.pbs_order] = [s.name, LimitProperty2.new(@limit),
                                 _INTL("{1}的个体值(范围：0-{2})。", s.name, @limit)]
...
class EVsProperty
  ...
  def set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      oldsetting[s.pbs_order] = 0 if !oldsetting[s.pbs_order]
      properties[s.pbs_order] = [s.name, LimitProperty2.new(@limit),
                                 _INTL("{1}的努力值(范围：0-{2})。", s.name, @limit)]
      data[s.pbs_order] = oldsetting[s.id]
      stat_ids[s.pbs_order] = s.id
    end
    loop do
      ...
      pbMessage(_INTL("总努力值（{1}）大于允许的（{2}），请修改。", evtotal, Pokemon::EV_LIMIT))
    end
    ret = {}
    stat_ids.each_with_index { |s, i| ret[s] = data[i] || 0 }
    return ret
...
module MapSizeProperty
  def self.set(settingname, oldsetting)
    oldsetting = [0, ""] if !oldsetting
    properties = [
      [_INTL("宽度"),         NonzeroLimitProperty.new(30), _INTL("此地图在地区地图方块中的宽度")],
      [_INTL("有效方块"), StringProperty,               _INTL("标记方块是否是这张地图的一部分（1=是一部分，0=不是一部分）")]
    ]
...
def chooseMapPoint(map, rgnmap = false)
  ...
  title = Window_UnformattedTextPokemon.newWithSize(
    _INTL("点击地图上一点。"), 0, Graphics.height - 64, Graphics.width, 64, viewport
  )
  title.z = 2
  if rgnmap
...
module MapCoordsFacingProperty
  def self.set(settingname, oldsetting)
    ...
    if chosenmap >= 0
      mappoint = chooseMapPoint(chosenmap)
      if mappoint
        facing = pbMessage(_INTL("选择面对的方向。"),
                           [_INTL("下"), _INTL("左"), _INTL("右"), _INTL("上")], -1)
...
module RegionMapCoordsProperty
  def self.set(_settingname, oldsetting)
    ...
    when 1
      ...
      selcmd = pbMessage(_INTL("选择地区地图"), cmds, -1)
...
module WeatherEffectProperty
  def self.set(_settingname, oldsetting)
    ...
    number = pbMessageChooseNumber(_INTL("设定天气概率"), params)
...
module PocketProperty
  def self.set(_settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("为此道具选择口袋"), commands, -1)
...
module BaseStatsProperty
  def self.set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      next if s.pbs_order < 0
      properties[s.pbs_order] = [_INTL("{1}", s.name), NonzeroLimitProperty.new(255),
                                 _INTL("宝可梦的{1}种族值", s.name)]
...
class GameDataPoolProperty
  def initialize(game_data, allow_multiple = true, auto_sort = false)
    if !GameData.const_defined?(game_data.to_sym)
      raise _INTL("在 module GameData 中找不到class {1}。", game_data.to_s)
    end
    @game_data = game_data
      ...
      when 0
        if cmd[1] >= 0   # Chose an entry
          ...
          if entry[0] == -1   # Add new move
            ...
            params.setCancelValue(-1)
            newlevel = pbMessageChooseNumber(_INTL("选择等级"), params)
            if newlevel >= 0
            ...
            case pbMessage("\\ts[]" + _INTL("对这个招式做什么？"),
                           [_INTL("修改等级"), _INTL("修改招式"), _INTL("删除"), _INTL("取消")], 4)
            when 0   # Change level
              params = ChooseNumberParams.new
              params.setRange(0, GameData::GrowthRate.max_level)
              params.setDefaultValue(entry[0])
              newlevel = pbMessageChooseNumber(_INTL("选择新等级"), params)
...
class EvolutionsProperty
  ...
  def edit_parameter(evo_method, value = nil)
    ...
    when String
      ret = pbMessageFreeText(_INTL("请输入一个值。"), ret || "", false, 250, Graphics.width)
      ret.strip!
      ...
      params.setCancelValue(-1)
      ret = pbMessageChooseNumber(_INTL("选择参数"), params)
      ret = nil if ret < 0
    end
      ...
      when 0
        if cmd[1] >= 0
          ...
          if entry[3] == -1   # Add new evolution path
            pbMessage(_INTL("选择进化的形态，方法和参数"))
            newspecies = pbChooseSpeciesList
            if newspecies
              newmethodindex = pbMessage(_INTL("选择进化方法"), @methods, -1)
              if newmethodindex >= 0
            ...
            case pbMessage("\\ts[]" + _INTL("对此进化做什么？"),
                           [_INTL("修改物种"), _INTL("修改方法"),
                            _INTL("修改参数"), _INTL("删除"), _INTL("取消")], 5)
            ...
            when 1   # Change method
              ...
              newmethodindex = pbMessage(_INTL("选择进化方法"), @methods, -1, nil, default_index)
              if newmethodindex >= 0
                newmethod = @evo_ids[newmethodindex]
                existing_evo = -1
                realcmds.length.times do |i|
...
module EncounterSlotProperty
  def self.set(setting_name, data)
    ...
    properties = [
      [_INTL("概率"),   NonzeroLimitProperty.new(999),       _INTL("选择出现的相对概率。")],
      [_INTL("物种"),       SpeciesFormProperty.new(data[1]),    _INTL("宝可梦的物种/形态。")],
      [_INTL("最低等级"), NonzeroLimitProperty.new(max_level), _INTL("此物种的最低等级(1-{1})", max_level)],
      [_INTL("最高等级"), NonzeroLimitProperty.new(max_level), _INTL("此物种的最高等级(1-{1})", max_level)]
    ]
