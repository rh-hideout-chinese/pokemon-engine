...
def pbWarpToMap
  mapid = pbListScreen(_INTL("切换地图"), MapLister.new(pbDefaultMap))
  if mapid > 0
    map = Game_Map.new
    map.setup(mapid)
    success = false
...
def pbDebugDayCare
  ...
  loop do
    if need_refresh
      ...
      day_care.slots.each_with_index do |slot, i|
        if slot.filled?
          ...
          commands.push(_INTL("[位置{1}] {2}", i, msg))
        else
          commands.push(_INTL("[位置{1}] 空", i))
        end
      end
      compat = $PokemonGlobal.day_care.get_compatibility
      if day_care.egg_generated
        commands.push(_INTL("[有宝可梦蛋]"))
      elsif compat > 0
        commands.push(_INTL("[会出现蛋]"))
      else
        commands.push(_INTL("[无法繁殖]"))
      end
      commands.push(_INTL("[到下周期步数：{1}]", 256 - day_care.step_counter))
    ...
    if cmd == commands.length - 2   # Egg
      compat = $PokemonGlobal.day_care.get_compatibility
      if compat == 0
        pbMessage(_INTL("宝可梦不能繁殖。"))
      else
        msg = _INTL("宝可梦可以繁殖。（和睦性 = {1}）", compat)
        # Show compatibility
        if day_care.egg_generated
          ...
          when 0   # Collect egg
            if $player.party_full?
              pbMessage(_INTL("队伍已满，无法收集宝可梦蛋。"))
            else
              DayCare.collect_egg
              pbMessage(_INTL("收集了{1}个宝可梦蛋。", $player.last_party.speciesName))
              need_refresh = true
            end
          when 1   # Clear egg
          ...
          case pbMessage("\\ts[]" + msg, [_INTL("制造宝可梦蛋"), _INTL("取消")], 2)
      ...
      case pbMessage("\\ts[]" + _INTL("修改到下一个周期的步数？"),
                     [_INTL("设置为1"), _INTL("设置为256"), _INTL("设置为其他值"), _INTL("取消")], 4)
      when 0   # Set to 1
      ...
      when 2   # Set to other value
        ...
        new_counter = pbMessageChooseNumber(_INTL("设置到下一个周期的步数(1-256)。"), params)
      ...
      if slot.filled?
        ...
        case pbMessage("\\ts[]" + _INTL("这个位置是空的。"),
                       [_INTL("存放"), _INTL("取消")], 2)
        when 0   # Deposit
          if $player.party.empty?
            pbMessage(_INTL("队伍为空，无法存放宝可梦。"))
          else
...
class SpriteWindow_DebugRoamers < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == self.itemCount - 2
      # Advance roaming
      self.shadowtext(_INTL("[所有宝可梦游走到新位置]"), rect.x, rect.y, nameWidth, rect.height)
    elsif index == self.itemCount - 1
      # Advance roaming
      self.shadowtext(_INTL("[清除当前所有游走宝可梦位置]"), rect.x, rect.y, nameWidth, rect.height)
    else
...
def pbExportAllAnimations
  begin
    ...
    if animations
      ...
      pbMessage(_INTL("所有动画都被提取并保存到\nAnimations文件夹了。"))
    else
      pbMessage(_INTL("没有动画可导出。"))
    end
  rescue
    p $!.message, $!.backtrace
...
def pbImportAllAnimations
  ...
  if animationFolders.length == 0
    pbMessage(_INTL("没有要导入的动画，请将动画放入Animations文件夹内。"))
  else
    msgwindow = pbCreateMessageWindow
    animations = pbLoadBattleAnimations
...
def pbDebugFixInvalidTiles
  ...
  Console.echo_h1(_INTL("检查{1}个地图是否有无效地图块", total_maps))
  mapData.mapinfos.keys.sort.each do |id|
    ...
    Console.echoln_li(_INTL("在地图{2}上发现{1}个错误地图块：{3}。", map_errors, id, mapData.mapinfos[id].name))
    total_errors += map_errors
    num_error_maps += 1
    mapData.saveMap(id)
  end
  if num_error_maps == 0
    Console.echo_h2(_INTL("检查完毕，没有发现错误。"), text: :green)
    pbMessage(_INTL("没有发现无效地图块。"))
  else
    echoln ""
    Console.echo_h2(_INTL("检查完毕，发现并修复了{1}个错误。", total_errors), text: :green)
    Console.echo_warn(_INTL("RMXP数据已被修改，请关闭RPG Maker XP以确保应用更改。"))
    echoln ""
    pbMessage(_INTL("在{2}个地图上发现并修复{1}个错误。", total_errors, num_error_maps))
    pbMessage(_INTL("请关闭RPG Maker XP以确保应用更改。"))
  end
end
