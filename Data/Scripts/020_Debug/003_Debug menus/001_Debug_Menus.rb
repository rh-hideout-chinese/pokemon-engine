...
module PokemonDebugMixin
  def pbPokemonDebug(pkmn, pkmnid, heldpoke = nil, settingUpBattle = false)
    ...
    command = 0
    loop do
      command = pbShowCommands(_INTL("对{1}做什么？", pkmn.name), commands.list, command)
      if command < 0
        parent = commands.getParent
        break if !parent
        commands.currentList = parent[0]
...
module Battle::DebugMixin
  ...
  def pbBattleDebugBattlerInfo(battler)
    ...
    ret += "\n"
    # Form number
    ret += _INTL("形态：{1}", battler.form)
    ret += "\n"
    # Level, gender, shininess
    ret += _INTL("等级{1}，{2}", battler.level,
                 (battler.pokemon.male?) ? "♂" : (battler.pokemon.female?) ? "♀" : _INTL("无性别"))
    ret += ", " + _INTL("闪光") if battler.pokemon.shiny?
    ret += "\n"
    # HP
    ...
    case battler.status
    when :SLEEP
      ret += " " + _INTL("（剩{1}轮）", battler.statusCount)
    when :POISON
      if battler.statusCount > 0
        ret += " " + _INTL("（剧毒，{1}/16）", battler.effects[PBEffects::Toxic])
      end
    end
    ret += "\n"
    # Stat stages
    ...
    ret += _INTL("能力阶段：{1}", (stages.empty?) ? "-" : stages.join(", "))
    ret += "\n"
    # Ability
    ret += _INTL("特性：{1}", (battler.ability) ? battler.abilityName : "-")
    ret += "\n"
    # Held item
    ret += _INTL("持有物：{1}", (battler.item) ? battler.itemName : "-")
    return ret
  end
  ...
  def pbBattleDebugPokemonInfo(pkmn)
    ...
    ret += "\n"
    # Form number
    ret += _INTL("形态：{1}", sp_data.form)
    ret += "\n"
    # Level, gender, shininess
    ret += _INTL("等级{1}，{2}", pkmn.level,
                 (pkmn.male?) ? "♂" : (pkmn.female?) ? "♀" : _INTL("无性别"))
    ret += ", " + _INTL("闪光") if pkmn.shiny?
    ret += "\n"
    # HP
    ...
    case pkmn.status
    when :SLEEP
      ret += " " + _INTL("（剩{1}轮）", pkmn.statusCount)
    when :POISON
      ret += " " + _INTL("（剧毒）") if pkmn.statusCount > 0
    end
    ret += "\n"
    # Ability
    ...
    ret += "\n"
    # Held item
    ret += _INTL("持有物：{1}", pkmn.item&.name || "-")
    return ret
  end
