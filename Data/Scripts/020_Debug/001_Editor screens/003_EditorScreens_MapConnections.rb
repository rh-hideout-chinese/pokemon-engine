...
class MapScreenScene
  ...
  def helpWindow
    ...
    helptext += _INTL("DEL：从画布中删除地图") + "\n"
    helptext += _INTL("S：转到另一张地图") + "\n"
    helptext += _INTL("单击以选择地图") + "\n"
    helptext += _INTL("双击: 编辑地图的元数据") + "\n"
    helptext += _INTL("拖动地图以移动它") + "\n"
    helptext += _INTL("方向键/拖动画布：在画布周围移动")
...
#   echoln "mousemove (#{mapid},#{x},#{y})"
  ...
  def update
    ...
    if Input.triggerex?(:A)
      ...
      id = chooseMapScreen(_INTL("去地图"), @currentmap)
      if id > 0
        @mapconns = generateConnectionData
        pbDisposeSpriteHash(@mapsprites)
        @mapsprites.clear
  ...
  def pbMapScreenLoop
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        break if pbConfirmMessage(_INTL("要退出编辑器吗？"))
      end
    end
  end
end
