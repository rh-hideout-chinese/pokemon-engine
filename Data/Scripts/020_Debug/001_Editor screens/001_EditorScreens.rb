...
def pbEncountersEditor
  ...
  help_window = Window_UnformattedTextPokemon.newWithSize(
    _INTL("编辑遭遇"), Graphics.width / 2, 0, Graphics.width / 2, 96
  )
  help_window.z = 99999
  ret = 0
  need_refresh = true
  loop do
    ...
    if ret == 0   # Add new encounter set
      ...
      if new_map_ID > 0
        new_version = LimitProperty2.new(999).set(_INTL("版本"), 0)
        if new_version && new_version >= 0
          if GameData::Encounter.exists?(new_map_ID, new_version)
            pbMessage(_INTL("地图{1}的版本{2}遭遇已存在。", new_map_ID, new_version))
          else
      ...
      when 2   # Delete
        if pbConfirmMessage(_INTL("删除地图{1}的版本{2}遭遇？", this_set[0], this_set[1]))
...
def pbEncounterMapVersionEditor(enc_data)
  ...
  loop do
    ...
    if ret == 0   # Edit map ID
      ...
      new_map_ID = pbListScreen(_INTL("选择一个地图"), MapLister.new(old_map_ID))
      if new_map_ID > 0 && new_map_ID != old_map_ID
        if GameData::Encounter.exists?(new_map_ID, enc_data.version)
          pbMessage(_INTL("地图{1}的版本{2}遭遇已存在。", new_map_ID, enc_data.version))
        else
      ...
      if new_version && new_version != old_version
        if GameData::Encounter.exists?(enc_data.map, new_version)
          pbMessage(_INTL("地图{1}的版本{2}遭遇已存在。", enc_data.map, new_version))
        else
          GameData::Encounter::DATA.delete(enc_data.id)
      ...
      when 1   # Copy
        ...
        if new_type_commands.length > 0
          chosen_type_cmd = pbMessage(_INTL("选择要粘贴的遭遇类型。"),
                                      new_type_commands, -1)
          if chosen_type_cmd >= 0
            new_type = new_types[chosen_type_cmd]
          ...
          pbMessage(_INTL("没有未使用的遭遇类型可以粘贴了。"))
        end
      when 2   # Delete
...
def pbEncounterTypeEditor(enc_data, enc_type)
  ...
  loop do
    if need_refresh
      ...
      commands.clear
      commands.push(_INTL("遭遇概率（每步）={1}%", enc_data.step_chances[enc_type] || 0))
      ...
      commands.push(_INTL("[添加新槽]"))
      need_refresh = false
    end
    ret = pbCommands2(list, commands, -1, ret)
    if ret == 0   # Edit step chance
      old_step_chance = enc_data.step_chances[enc_type] || 0
      new_step_chance = LimitProperty.new(255).set(_INTL("遭遇概率（每步）"), old_step_chance)
      ...
      when 1   # Copy
        ...
        need_refresh = true
      when 2   # Delete
        if pbConfirmMessage(_INTL("删除这个遭遇槽？"))
...
def pbTrainerTypeEditorNew(default_name)
  # Choose a name
  name = pbMessageFreeText(_INTL("请输入此训练家类型的名称。"),
  ...
  if GameData::TrainerType.exists?(id)
    pbMessage(_INTL("未能创建训练家类型，请输入不同的名称。"))
    return nil
  end
  # Choose a gender
  gender = pbMessage(_INTL("训练家是男性，女性，还是未知性别？"),
                     [_INTL("男性"), _INTL("女性"), _INTL("未知")], 0)
  # Choose a base money value
  params = ChooseNumberParams.new
  params.setRange(0, 255)
  params.setDefaultValue(30)
  base_money = pbMessageChooseNumber(_INTL("设置击败训练家赢得 金钱的倍数。"), params)
  # Construct trainer type hash
  tr_type_hash = {
    :id         => id.to_sym,
    :name       => name,
  ...
  GameData::TrainerType.save
  pbConvertTrainerData
  pbMessage(_INTL("训练家类型已创建（ID :{1}）", name, id.to_s))
  pbMessage(_INTL("把训练家的图形（trainer{1:03d}.png或者trainer{2:s}.png）放到Graphics/Characters里，否则训练家将是空白的。", id.to_s))
  return id.to_sym
end
...
module TrainerBattleProperty
  ...
  def self.set(settingname, oldsetting)
    ...
    properties = [
      [_INTL("训练家类型"), TrainerTypeProperty,     _INTL("此训练家的训练家类 型名称")],
      [_INTL("训练家名称"), StringProperty,          _INTL("训练家的名称。")],
      [_INTL("版本"),      LimitProperty.new(9999), _INTL("用于区分具有相同名 称和训练家类型的训 练家的编号。")],
      [_INTL("战败文本"),    StringProperty,          _INTL("当训练家被击败时显 示的信息。")]
    ]
    Settings::MAX_PARTY_SIZE.times do |i|
      properties.push([_INTL("宝可梦{1}", i + 1), TrainerPokemonProperty, _INTL("训练家拥有的宝可梦 。")])
    end
    NUM_ITEMS.times do |i|
      properties.push([_INTL("道具{1}", i + 1), ItemProperty, _INTL("训练家在战斗中使用 的道具。")])
    end
...
def pbTrainerBattleEditor
  modified = false
  pbListScreenBlock(_INTL("训练家"), TrainerBattleLister.new(0, true)) do |button, trainer_id|
    if trainer_id
      case button
      when Input::ACTION
      ...
      when Input::USE
        if trainer_id.is_a?(Array)   # Edit existing trainer
          ...
          loop do
            ...
            if !data[0]
              pbMessage(_INTL("无法保存，没有选择训练家类型。"))
            elsif !data[1] || data[1].empty?
              pbMessage(_INTL("无法保存，没有输入名称。"))
          ...
          tr_type = nil
          ret = pbMessage(_INTL("首先，请定义新训练家的类型。"),
                          [_INTL("使用存在类型"),
                           _INTL("创建新类型"),
                           _INTL("取消")], 3)
          case ret
          ...
          next if !tr_type
          tr_name = pbMessageFreeText(_INTL("输入训练家的名字。"), "", false, 30)
          next if nil_or_empty?(tr_name)
          tr_version = pbGetFreeTrainerParty(tr_type, tr_name)
          if tr_version < 0
            pbMessage(_INTL("没有创建这种类型和名称的训练家的位置了。"))
            next
          end
  ...
  if modified && pbConfirmMessage(_INTL("保存修改吗？"))
...
module TrainerPokemonProperty
  def self.set(settingname, initsetting)
    ...
    pkmn_properties = [
      ...
      [_INTL("昵称"),          StringProperty,                          _INTL("宝可梦的昵称。")],
      [_INTL("形态"),          LimitProperty2.new(999),                 _INTL("宝可梦的形态。")],
      [_INTL("性别"),        GenderProperty,                          _INTL("宝可梦的性别。")],
      [_INTL("闪光"),         BooleanProperty2,                        _INTL("如果设置为真，则其为闪光宝可梦。")],
      [_INTL("超闪光"),    BooleanProperty2,                        _INTL("宝可梦是否为超闪光。（闪光并带有特殊的闪光动画）")],
      [_INTL("黑暗"),        BooleanProperty2,                        _INTL("如果设置为真，则其为黑暗宝可梦。")]
    ]
    Pokemon::MAX_MOVES.times do |i|
      pkmn_properties.push([_INTL("招式{1}", i + 1),
                            MovePropertyForSpecies.new(oldsetting), _INTL("宝可梦知道的招式，野生宝可梦请将所有招式留空。（使用 Z 键删除）")])
    end
    pkmn_properties.concat(
      [[_INTL("特性"),       AbilityProperty,                         _INTL("宝可梦的特性，会覆盖特性索引。")],
       [_INTL("特性索引"), LimitProperty2.new(99),                  _INTL("特性的索引。 \n0=第一特性\n1=第二特性\n2+=隐藏特性")],
       [_INTL("携带道具"),     ItemProperty,                            _INTL("宝可梦携带的道具")],
       [_INTL("性格"),        GameDataProperty.new(:Nature),           _INTL("宝可梦的性格。")],
       [_INTL("个体值"),           IVsProperty.new(Pokemon::IV_STAT_LIMIT), _INTL("宝可梦各项能力的个体值。")],
       [_INTL("努力值"),           EVsProperty.new(Pokemon::EV_STAT_LIMIT), _INTL("宝可梦各项能力的努力值。")],
       [_INTL("亲密度"),     LimitProperty2.new(255),                 _INTL("宝可梦的初始亲密度（0-255）")],
       [_INTL("精灵球"),     BallProperty.new(oldsetting),            _INTL("收服此宝可梦的精灵球。")]]
    )
...
def pbEditMetadata
  ...
  if pbPropertyList(_INTL("全局元数据"), data, properties, true)
...
def pbEditPlayerMetadata(player_id = 1)
  ...
  if player_id < 1
    ...
    pbMessage(_INTL("未找到玩家角色{1}的元数据。", player_id))
    return
  end
  data = []
...
def pbItemEditorNew(default_name)
  # Choose a name
  name = pbMessageFreeText(_INTL("请输入此道具的名称。"),
                           (default_name) ? default_name.gsub(/_+/, " ") : "", false, 30)
  if nil_or_empty?(name)
  ...
  if GameData::Item.exists?(id)
    pbMessage(_INTL("未能创建道具，请输入不同的名称。"))
    return
  end
  # Choose a pocket
  ...
  return if pocket == 0
  # Choose a price
  price = LimitProperty.new(999_999).set(_INTL("此道具的购买价格。"), -1)
  return if price == -1
  ...
  GameData::Item.save
  Compiler.write_items
  pbMessage(_INTL("道具{1}已创建（ID：{2}）", name, id.to_s))
  pbMessage(_INTL("请将训练家的图形：（{1}.png)）放在 Graphics/Trainers 中，否则此训练家将是空白的。", id.to_s))
end
...
def pbPokemonEditor
  ...
  pbListScreenBlock(_INTL("宝可梦物种"), SpeciesLister.new(0, false)) do |button, species|
    if species
      case button
      when Input::ACTION
  ...
  loop do
    ...
    when 0
      if cmd[1] >= 0   # Edit entry
        case pbMessage("\\ts[]" + _INTL("对此条目做什么？"),
                       [_INTL("修改物种"), _INTL("清除"),
...
def pbRegionalDexEditorMain
  ...
  title = Window_UnformattedTextPokemon.newWithSize(
    _INTL("地区图鉴编辑器"), Graphics.width / 2, 0, Graphics.width / 2, 64, viewport
  )
  title.z = 2
  info = Window_AdvancedTextPokemon.newWithSize(
  ...
  loop do
    ...
    if refresh_list
      ...
      dex_lists.each_with_index do |list, i|
        commands.push(_INTL("图鉴{1}（大小{2}）", i + 1, list.length))
      end
      refresh_list = false
    end
    # Choose to do something
    ...
    when 0   # Clicked on a command/Dex
      if cmd[1] == 0   # Add new Dex
        case pbMessage(_INTL("填满这个新的图鉴？"),
                       [_INTL("留空"), _INTL("全国图鉴"),
                        _INTL("对图鉴做什么？"), _INTL("取消")], 4)
        when 0   # Leave blank
          dex_lists.push([])
          refresh_list = true
        ...
        case pbMessage("\\ts[]" + _INTL("对此图鉴做什么？"),
...
def pbAnimationsOrganiser
  ...
  cmdwin.z        = 2
  title = Window_UnformattedTextPokemon.newWithSize(
    _INTL("整理动画"), Graphics.width / 2, 0, Graphics.width / 2, 64, viewport
  )
  title.z = 2
  info = Window_AdvancedTextPokemon.newWithSize(
    _INTL("Z+上/下：交换\nZ+左：删除\nZ+右：插入"),
