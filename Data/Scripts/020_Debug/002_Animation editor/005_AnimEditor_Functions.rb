module BattleAnimationEditor
  ...
  def pbChangeMaximum(canvas)
    ...
    sliderwin2.addSlider(_INTL("帧："), 1, 1000, canvas.animation.length)
    okbutton = sliderwin2.addButton(_INTL("确认"))
  ...
  def pbAnimName(animation, cmdwin)
    ...
    okbutton = window.addButton(_INTL("确认"))
  ...
  def pbAnimList(animations, canvas, animwin)
    ...
    helpwindow = Window_UnformattedTextPokemon.newWithSize(
      _INTL("Enter：加载/重命名动画\nEsc：取消"),
      320, 0, 320, 128, canvas.viewport
    )
    maxsizewindow = ControlWindow.new(0, 416, 320, 32 * 3)
    maxsizewindow.addSlider(_INTL("总动画："), 1, 2000, animations.length)
    maxsizewindow.addButton(_INTL("调整动画列表"))
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) && animations.length > 0
        ...
        when 1   # Rename
          ...
          cmdwin.refresh
        when 2   # Delete
          if pbConfirmMessage(_INTL("你确定要删除此动画吗？"))
  ...
  def pbChooseNum(cel)
    ...
    sliderwin2.z = 99999
    sliderwin2.addLabel(_INTL("旧编号：{1}", cel))
    sliderwin2.addSlider(_INTL("新编号："), 2, PBAnimation::MAX_SPRITES, cel)
    okbutton = sliderwin2.addButton(_INTL("确认"))
    cancelbutton = sliderwin2.addButton(_INTL("取消"))
    loop do
      Graphics.update
  ...
  def pbSetTone(cel, previewsprite)
    ...
    sliderwin2.z = 99999
    sliderwin2.addSlider(_INTL("红色偏移量："), -255, 255, cel[AnimFrame::TONERED])
    sliderwin2.addSlider(_INTL("绿色偏移量："), -255, 255, cel[AnimFrame::TONEGREEN])
    sliderwin2.addSlider(_INTL("蓝色偏移量："), -255, 255, cel[AnimFrame::TONEBLUE])
  ...
  def pbSetFlash(cel, previewsprite)
    sliderwin2 = ControlWindow.new(0, 0, 320, 320)
    sliderwin2.z = 99999
    sliderwin2.addSlider(_INTL("红："), 0, 255, cel[AnimFrame::COLORRED])
    sliderwin2.addSlider(_INTL("绿："), 0, 255, cel[AnimFrame::COLORGREEN])
    sliderwin2.addSlider(_INTL("蓝："), 0, 255, cel[AnimFrame::COLORBLUE])
    sliderwin2.addSlider(_INTL("通道透明度："), 0, 255, cel[AnimFrame::COLORALPHA])
    okbutton = sliderwin2.addButton(_INTL("确认"))
    cancelbutton = sliderwin2.addButton(_INTL("取消"))
    loop do
      previewsprite.tone.set(sliderwin2.value(0), sliderwin2.value(1),
  ...
  def pbCellProperties(canvas)
    ...
    set3 = sliderwin2.addSlider(_INTL("缩放 X："), 5, 1000, cel[AnimFrame::ZOOMX])
    ...
    set6 = sliderwin2.addSlider(_INTL("元素透明度："), 0, 255, cel[AnimFrame::OPACITY])
    set7 = sliderwin2.addSlider(_INTL("混合："), 0, 2, cel[AnimFrame::BLENDTYPE])
    set8 = sliderwin2.addTextSlider(_INTL("翻转："), [_INTL("假"), _INTL("真")], cel[AnimFrame::MIRROR])
    prio = [_INTL("反面"), _INTL("正面"), _INTL("焦点后"), _INTL("焦点前")]
    set9 = sliderwin2.addTextSlider(_INTL("优先级："), prio, cel[AnimFrame::PRIORITY] || 1)
    foc = [_INTL("使用者"), _INTL("目标"), _INTL("使用者和目标"), _INTL("屏幕")]
    curfoc = [3, 1, 0, 2, 3][cel[AnimFrame::FOCUS] || canvas.animation.position || 4]
    set10 = sliderwin2.addTextSlider(_INTL("焦点："), foc, curfoc)
  ...
  def pbTimingList(canvas)
    ...
    commands[cmdNewBG = commands.length] = _INTL("添加：设置背景图形...")
    commands[cmdEditBG = commands.length] = _INTL("添加：编辑背景色/位置...")
    ...
    framewindow.addSlider(_INTL("帧："), 1, canvas.animation.length, canvas.currentframe + 1)
    framewindow.addButton(_INTL("设置帧"))
    framewindow.addButton(_INTL("删除时间"))
    ...
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        if redrawcmds
          ...
          cmdwin.commands[cmdNewBG = cmdwin.commands.length] = _INTL("添加：设置背景图形...")
          cmdwin.commands[cmdEditBG = cmdwin.commands.length] = _INTL("添加：编辑背景色/位置...")
  ...
  def pbSelectSE(canvas, audio)
    ...
    displayname = (filename != "") ? filename : _INTL("<使用者叫声>")
    animfiles = []
    ret = false
  ...
  #    animfiles.concat(Dir.glob("*.mp3"))
    ...
    animfiles = [_INTL("[播放使用者叫声]")] + animfiles
    cmdwin = pbListWindow(animfiles, 320)
    cmdwin.height = 480
    cmdwin.opacity = 200
    cmdwin.viewport = canvas.viewport
    ...
    maxsizewindow.addSlider(_INTL("音量："), 0, 100, audio.volume)
    maxsizewindow.addSlider(_INTL("音调："), 20, 250, audio.pitch)
    maxsizewindow.addButton(_INTL("播放声音"))
    maxsizewindow.addButton(_INTL("停止声音"))
    maxsizewindow.addButton(_INTL("确认"))
  ...
  def pbSelectBG(canvas, timing)
    ...
    cmdErase = -1
    animfiles = []
    animfiles[cmdErase = animfiles.length] = _INTL("[擦除背景图形]")
    ret = false
  ...
  #    animfiles.concat(Dir.glob("*.bmp"))
    ...
    maxsizewindow.addSlider(_INTL("元素透明度："), 0, 255, timing.opacity || 0)
    maxsizewindow.addSlider(_INTL("红："), 0, 255, timing.colorRed || 0)
    maxsizewindow.addSlider(_INTL("绿："), 0, 255, timing.colorGreen || 0)
    maxsizewindow.addSlider(_INTL("蓝："), 0, 255, timing.colorBlue || 0)
    maxsizewindow.addSlider(_INTL("通道透明度："), 0, 255, timing.colorAlpha || 0)
    maxsizewindow.addButton(_INTL("确认"))
    maxsizewindow.addButton(_INTL("取消"))
    maxsizewindow.opacity = 200
    maxsizewindow.viewport = canvas.viewport
  ...
  def pbEditBG(canvas, timing)
    ...
    maxsizewindow.addOptionalSlider(_INTL("元素透明度："), 0, 255, timing.opacity || 0)
    maxsizewindow.addOptionalSlider(_INTL("红："), 0, 255, timing.colorRed || 0)
    maxsizewindow.addOptionalSlider(_INTL("绿："), 0, 255, timing.colorGreen || 0)
    maxsizewindow.addOptionalSlider(_INTL("蓝："), 0, 255, timing.colorBlue || 0)
    maxsizewindow.addOptionalSlider(_INTL("通道透明度："), 0, 255, timing.colorAlpha || 0)
    maxsizewindow.addButton(_INTL("确认"))
    maxsizewindow.addButton(_INTL("取消"))
  ...
  def pbTweening(canvas)
    ...
    s1set2 = sliderwin2.addSlider(_INTL("手绘第一帧："), 0, PBAnimation::MAX_SPRITES - 1, 0)
    s1set3 = sliderwin2.addSlider(_INTL("手绘最后一帧："), 0, PBAnimation::MAX_SPRITES - 1, PBAnimation::MAX_SPRITES - 1)
  ...
  def pbCellBatch(canvas)
    ...
    s1set2 = sliderwin1.addSlider(_INTL("手绘第一帧："), 0, PBAnimation::MAX_SPRITES - 1, 0)
    s1set3 = sliderwin1.addSlider(_INTL("手绘最后一帧："), 0, PBAnimation::MAX_SPRITES - 1, PBAnimation::MAX_SPRITES - 1)
  ...
  def pbEntireSlide(canvas)
    ...
    sliderwin2.addSlider(_INTL("第一帧："), 1, canvas.animation.length, 1)
    sliderwin2.addSlider(_INTL("最后一帧："), 1, canvas.animation.length, canvas.animation.length)
    sliderwin2.addSlider(_INTL("水平运动"), -500, 500, 0)
    sliderwin2.addSlider(_INTL("垂直运动"), -500, 500, 0)
    okbutton = sliderwin2.addButton(_INTL("确认"))
    cancelbutton = sliderwin2.addButton(_INTL("取消"))
    sliderwin2.opacity = 200
    loop do
  ...
  def animationEditorMain(animation)
    ...
    sidewin.addButton(_INTL("SE和BG..."))
    sidewin.addButton(_INTL("渲染焦点……"))
    sidewin.addSpace
    sidewin.addButton(_INTL("粘贴最后"))
    sidewin.addButton(_INTL("复制帧……"))
    sidewin.addButton(_INTL("清空帧……"))
    sidewin.addButton(_INTL("插值……"))
    sidewin.addButton(_INTL("帧批处理……"))
    sidewin.addButton(_INTL("幻灯片……"))
    sidewin.addSpace
    sidewin.addButton(_INTL("播放动画"))
    sidewin.addButton(_INTL("播放反动画"))
    ...
    sliderwin.addButton(_INTL("动画图集表"))
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        if pbConfirmMessage(_INTL("保存修改吗？"))
          save_data(animation, "Data/PkmnAnimations.rxdata")
        end
        if pbConfirmMessage(_INTL("要退出编辑器吗？"))
      ...
      if sidewin.changed?(1)
        positions = [_INTL("使用者"), _INTL("目标"), _INTL("使用者和目标"), _INTL("窗口")]
