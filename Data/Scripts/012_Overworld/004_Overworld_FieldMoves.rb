...
def pbCheckHiddenMoveBadge(badge = -1, showmsg = true)
  ...
  pbMessage(_INTL("抱歉，这需要拥有对应的徽章。")) if showmsg
  return false
end
...
def pbCut
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_CUT, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这棵树看起来可以砍掉！"))
    return false
  end
  if pbConfirmMessage(_INTL("这棵树看起来可以砍掉！\n你想砍掉它吗？"))
    $stats.cut_count += 1
...
HiddenMoveHandlers::CanUseMove.add(:CUT, proc { |move, pkmn, showmsg|
  ...
  if !facingEvent || !facingEvent.name[/cuttree/i]
    pbMessage(_INTL("你不能在这里使用。")) if showmsg
    next false
  end
  next true
})
...
HiddenMoveHandlers::CanUseMove.add(:DIG, proc { |move, pkmn, showmsg|
  ...
  if !$game_player.can_map_transfer_with_follower?
    pbMessage(_INTL("你不能在与他人同行时这样做。")) if showmsg
    next false
  end
  next true
})
...
def pbDive
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这里的水很深，宝可梦也许可以潜入水中。"))
    return false
  end
...
def pbSurfacing
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_DIVE, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("光从上方洒下，宝可梦也许可以从这浮出水面。"))
    return false
  end
...
def pbCanFly?(pkmn = nil, show_messages = false)
  ...
  if !$game_player.can_map_transfer_with_follower?
    pbMessage(_INTL("你不能在与他人同行时这样做。")) if show_messages
    return false
  end
  if !$game_map.metadata&.outdoor_map
    pbMessage(_INTL("你不能在这里使用。")) if show_messages
    return false
  end
  return true
end
...
def pbHeadbutt(event = nil)
  ...
  movefinder = $player.get_pokemon_with_move(move)
  if !$DEBUG && !movefinder
    pbMessage(_INTL("也许有宝可梦在这棵树上，也许有宝可梦可以晃晃它。"))
    return false
  end
  if pbConfirmMessage(_INTL("也许有宝可梦在这棵树上。你想用头锤吗？"))
...
def pbRockSmash
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_ROCKSMASH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这是一块岩石，宝可梦也许能破坏它。"))
    return false
  end
...
def pbStrength
  if $PokemonMap.strengthUsed
    pbMessage(_INTL("怪力使你可以移动巨石了。"))
    return false
  end
  move = :STRENGTH
  movefinder = $player.get_pokemon_with_move(move)
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_STRENGTH, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("这是一块巨石，但宝可梦也许能移动它。"))
    return false
  end
  pbMessage(_INTL("这是一块巨石，但宝可梦也许能移动它。") + "\1")
...
def pbSurf
  ...
  if pbConfirmMessage(_INTL("水是深蓝色的……要使用冲浪吗？"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1}使出了{2}！", speciesname, GameData::Move.get(move).name))
    pbCancelVehicles
    pbHiddenMoveAnimation(movefinder)
...
HiddenMoveHandlers::CanUseMove.add(:SURF, proc { |move, pkmn, showmsg|
  ...
  if $PokemonGlobal.surfing
    pbMessage(_INTL("你已经冲浪了。")) if showmsg
    next false
  end
...
def pbWaterfall
  ...
  if !pbCheckHiddenMoveBadge(Settings::BADGE_FOR_WATERFALL, false) || (!$DEBUG && !movefinder)
    pbMessage(_INTL("伴随着震耳欲聋的轰鸣声，瀑布倾泻而下。"))
    return false
  end
  if pbConfirmMessage(_INTL("这是一个瀑布，要使用攀瀑吗？"))
    speciesname = (movefinder) ? movefinder.name : $player.name
    pbMessage(_INTL("{1}使出了{2}！", speciesname, GameData::Move.get(move).name))
    pbHiddenMoveAnimation(movefinder)
    pbAscendWaterfall
