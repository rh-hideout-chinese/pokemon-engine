...
def setBattleRule(*args)
  ...
  raise _INTL("参数{1}后面应有一个变量。", r) if r
end
...
module BattleCreationHelperMethods
  ...
  def skip_battle(outcome_variable, trainer_battle = false)
    pbMessage(_INTL("跳过对战……")) if !trainer_battle && $player.pokemon_count > 0
    pbMessage(_INTL("跳过对战……")) if trainer_battle && $DEBUG
    pbMessage(_INTL("胜利之后……")) if trainer_battle && $player.able_pokemon_count > 0
...
class WildBattle
  ...
  def self.generate_foes(*args)
    ...
    args.each do |arg|
      case arg
      when Pokemon
        raise _INTL("已提供物种{1}，但未提供等级。", species_id) if species_id
        ret.push(arg)
      when Array
        raise _INTL("已提供物种{1}，但未提供等级。", species_id) if species_id
        ...
        if species_id   # Expecting level
          if !arg.is_a?(Integer) || !(1..GameData::GrowthRate.max_level).include?(arg)
            raise _INTL("应为等级(1..{1})，但{2}不是数字或不是有效值。", GameData::GrowthRate.max_level, arg)
          end
    ...
    raise _INTL("已提供物种{1}，但没有等级。", species_id) if species_id
    return ret
  end
end
...
class TrainerBattle
  ...
  def self.generate_foes(*args)
    ...
    args.each_with_index do |arg, i|
      case arg
      when NPCTrainer
        raise _INTL("已提供训练家类型{1}，但未提供训练家名称。", trainer_type) if trainer_type
      ...
      when Array   # [trainer type, trainer name, version number, speech (optional)]
        ...
        if trainer_name   # Expecting version number
          if !arg.is_a?(Integer) || arg < 0
            raise _INTL("需要训练家的版本为0及以上，但{1}不是数字或不是有效值。", arg)
          end
          ...
          raise _INTL("数据为“{1}、{2}、{3}”的训练家未定义。", trainer_type, trainer_name, arg) if !trainer
          EventHandlers.trigger(:on_trainer_load, trainer)
          trainer_array.push(trainer)
          foe_items.push(trainer.items)
          party_starts.push(pokemon_array.length)
          ...
          if !arg.is_a?(String) || arg.strip.empty?
            raise _INTL("需要训练家名称，“{1}”不是有效的名称。", arg)
          end
          if args[i + 1].is_a?(Integer)   # Version number is next
            ...
            raise _INTL("数据为“{1}、{2}”的训练家未定义。", trainer_type, arg) if !trainer
            EventHandlers.trigger(:on_trainer_load, trainer)
            trainer_array.push(trainer)
            foe_items.push(trainer.items)
            party_starts.push(pokemon_array.length)
