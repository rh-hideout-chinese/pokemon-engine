...
def pbFishing(hasEncounter, rodType = 1)
  ...
  loop do
    ...
    if pbWaitMessage(msgWindow, time)
      pbFishingEnd { pbMessageDisplay(msgWindow, _INTL("没有任何动静……")) }
      break
    end
    if hasEncounter && rand(100) < biteChance
      ...
      if !pbWaitForInput(msgWindow, message + "\n" + _INTL("啊！咬钩了！"), duration)
        pbFishingEnd { pbMessageDisplay(msgWindow, _INTL("宝可梦逃跑了……")) }
        break
      end
      if Settings::FISHING_AUTO_HOOK || rand(100) < hookChance
        pbFishingEnd do
          pbMessageDisplay(msgWindow, _INTL("宝可梦出现了！")) if !Settings::FISHING_AUTO_HOOK
        end
        ret = true
        break
      end
