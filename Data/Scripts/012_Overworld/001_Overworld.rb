...
EventHandlers.add(:on_frame_update, :low_battery_warning,
  proc {
    ...
    pbMessage(_INTL("已检测到电量不足。你应该尽快保存游戏，以免失去当前的进度。"))
  }
)
...
def pbItemBall(item, quantity = 1)
  ...
  if $bag.add(item, quantity)   # If item can be picked up
    ...
    if item == :DNASPLICERS
      pbMessage("\\me[#{meName}]" + _INTL("你捡到了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
    elsif item.is_machine?   # TM or HM
      if quantity > 1
        pbMessage("\\me[Machine get]" + _INTL("你捡到了{1}个\\c[1]{2}{3}\\c[0]！",
                                              quantity, itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
      else
        pbMessage("\\me[Machine get]" + _INTL("你捡到了\\c[1]{1}{2}\\c[0]！",
                                              itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
      end
    elsif quantity > 1
      pbMessage("\\me[#{meName}]" + _INTL("你捡到了个\\c[1]{2}\\c[0]！", quantity, itemname) + "\\wtnp[40]")
  ...
  if item.is_machine?   # TM or HM
    if quantity > 1
      pbMessage(_INTL("你捡到了个{1}\\c[1]{2} {3}\\c[0]！", quantity, itemname, GameData::Move.get(move).name))
    else
      pbMessage(_INTL("你捡到了\\c[1]{1}{2}\\c[0]！", itemname, GameData::Move.get(move).name))
    end
  elsif quantity > 1
    pbMessage(_INTL("你捡到了{1}个\\c[1]{2}\\c[0]！", quantity, itemname))
  elsif itemname.starts_with_vowel?
    pbMessage(_INTL("你捡到了\\c[1]{1}\\c[0]！", itemname))
  else
    pbMessage(_INTL("你捡到了\\c[1]{1}\\c[0]！", itemname))
  end
  pbMessage(_INTL("但是背包满了……"))
  return false
end
...
def pbReceiveItem(item, quantity = 1)
  ...
  meName = (item.is_key_item?) ? "Key item get" : "Item get"
  if item == :DNASPLICERS
    pbMessage("\\me[#{meName}]" + _INTL("你获得了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
  elsif item.is_machine?   # TM or HM
    if quantity > 1
      pbMessage("\\me[Machine get]" + _INTL("你获得了{1}个\\c[1]{2}{3}\\c[0]！",
                                            quantity, itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
    else
      pbMessage("\\me[Machine get]" + _INTL("你获得了\\c[1]{1}{2}\\c[0]！",
                                            itemname, GameData::Move.get(move).name) + "\\wtnp[70]")
    end
  elsif quantity > 1
    pbMessage("\\me[#{meName}]" + _INTL("你获得了个{1}\\c[1]{2}\\c[0]！", quantity, itemname) + "\\wtnp[40]")
  elsif itemname.starts_with_vowel?
    pbMessage("\\me[#{meName}]" + _INTL("你获得了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
  else
    pbMessage("\\me[#{meName}]" + _INTL("你获得了\\c[1]{1}\\c[0]！", itemname) + "\\wtnp[40]")
...
def pbBuyPrize(item, quantity = 1)
  ...
  pbMessage("\\CN" + _INTL("你将{1}放入<icon=bagPocket{2}>\\c[1]{3}\\c[0]口袋。",
