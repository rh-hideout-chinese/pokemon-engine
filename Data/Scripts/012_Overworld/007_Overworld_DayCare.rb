...
class DayCare
  ...
  def self.deposit(party_index)
    ...
    raise _INTL("队伍中索引{1}处没有宝可梦。", party_index) if pkmn.nil?
  ...
  def self.withdraw(index)
    ...
    slot = day_care[index]
    if !slot.filled?
      raise _INTL("位置{1}未找到宝可梦。", index)
    elsif $player.party_full?
      raise _INTL("队伍里没有能给宝可梦的位置了。")
    end
  ...
  def self.collect_egg
    ...
    raise _INTL("无法生成宝可梦蛋。") if egg.nil?
    raise _INTL("队伍里没有能给蛋的位置了。") if $player.party_full?
  ...
  def pokemon_pair
    ...
    raise _INTL("找不到2只寄存的宝可梦。") if pkmn2.nil?
    return pkmn1, pkmn2
  end
