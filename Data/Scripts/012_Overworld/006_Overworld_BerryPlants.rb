...
def pbBerryPlant
  ...
  if berry_plant.grown?
    ...
    when 1   # X planted
      ...
      if berry_name.starts_with_vowel?
        pbMessage(_INTL("这里种着{1}。", berry_name))
      else
        pbMessage(_INTL("这里种着{1}。", berry_name))
      end
    when 2   # X sprouted
      this_event.turn_down   # Stop the event turning towards the player
      pbMessage(_INTL("{1}已经发芽了。", berry_name))
    when 3   # X taller
      this_event.turn_left   # Stop the event turning towards the player
      pbMessage(_INTL("{1}长得很大了。", berry_name))
    else     # X flowering
      ...
      if Settings::NEW_BERRY_PLANTS
        ...
        when 4
          pbMessage(_INTL("{1}长得好极了！", berry_name))
        when 3
          pbMessage(_INTL("{1}的花开得很漂亮！", berry_name))
        when 2
          pbMessage(_INTL("{1}的花开得真美丽！", berry_name))
        when 1
          pbMessage(_INTL("{1}的花开得很可爱！", berry_name))
        else
          pbMessage(_INTL("{1}开花了！", berry_name))
        end
      end
    ...
    GameData::BerryPlant::WATERING_CANS.each do |item|
      ...
      break if !pbConfirmMessage(_INTL("想用{1}浇点水吗？",
                                       GameData::Item.get(item).name))
      berry_plant.water
      pbMessage("\\se[Water berry plant]" + _INTL("{1}浇了水。", $player.name) + "\\wtnp[40]")
      if Settings::NEW_BERRY_PLANTS
        pbMessage(_INTL("它看起来很开心！"))
      else
        pbMessage(_INTL("植物似乎很高兴。"))
      end
      break
    end
    return
  ...
  if Settings::NEW_BERRY_PLANTS
    ...
    if berry_plant.mulch_id
      pbMessage(_INTL("已经撒下了{1}。", GameData::Item.get(berry_plant.mulch_id).name))
    else
      case pbMessage(_INTL("泥土看起来相当的松软。"),
                     [_INTL("施肥"), _INTL("种植"), _INTL("返回")], -1)
      when 0   # Fertilize
        mulch = nil
        pbFadeOutIn do
        ...
        if mulch_data.is_mulch?
          ...
          $bag.remove(mulch)
          pbMessage(_INTL("{1}散落在土壤中。", mulch_data.name))
        else
          pbMessage(_INTL("这不能使土壤变得肥沃！"))
          return
        end
      when 1   # Plant Berry
        ask_to_plant = false
    ...
    # Old mechanics
    return if !pbConfirmMessage(_INTL("非常松软肥沃的泥土，想要种植树果吗？"))
    ask_to_plant = false
  end
  if !ask_to_plant || pbConfirmMessage(_INTL("想要种植树果吗？"))
    pbFadeOutIn do
      scene = PokemonBag_Scene.new
      screen = PokemonBagScreen.new(scene, $bag)
...
def pbPickBerry(berry, qty = 1)
  ...
  if qty > 1
    message = _INTL("这里有{1}个\\c[1]{2}\\c[0]！\n要摘下吗？", qty, berry_name)
  else
    message = _INTL("这里有1个\\c[1]{1}\\c[0]！\n要摘下吗？", berry_name)
  end
  return false if !pbConfirmMessage(message)
  if !$bag.can_add?(berry, qty)
    pbMessage(_INTL("太糟糕了……\n背包已经满了……"))
    return false
  end
  ...
  $bag.add(berry, qty)
  if qty > 1
    pbMessage("\\me[Berry get]" + _INTL("摘下了个{1}\\c[1]{2}\\c[0]。", qty, berry_name) + "\\wtnp[30]")
  else
    pbMessage("\\me[Berry get]" + _INTL("摘下了\\c[1]{1}\\c[0]。", berry_name) + "\\wtnp[30]")
  end
  pocket = berry.pocket
  pbMessage(_INTL("你将{1}放入<icon=bagPocket{2}>\\c[1]{3}\\c[0]口袋。",
                  berry_name, pocket, PokemonBag.pocket_names[pocket - 1]) + "\1")
  if Settings::NEW_BERRY_PLANTS
    pbMessage(_INTL("土壤又变得松软。"))
  else
    pbMessage(_INTL("土壤又变得松软肥沃。"))
  end
