...
class PokemonPokedexInfo_Scene
    ...
    GameData::Species.each do |sp|
      ...
      if sp.single_gendered?
        ...
        2.times do |real_gndr|
          ...
          ret.push([sp.form_name || _INTL("形态"), 0, sp.form])
          break
        end
    ...
    ret.each do |entry|
      if entry[0]   # Alternate forms, and form 0 if no gender differences
        ...
        when 1 then entry[0] = _INTL("雌性")
          ...
          entry[0] = (multiple_forms) ? _INTL("形态") : _INTL("无性别")
        end
      end
  ...
  def drawPageInfo
    ...
    textpos = [
      [_INTL("{1}{2} {3}", indexText, " ", species_data.name),
       246, 52, :left, Color.new(248, 248, 248), Color.black]
    ]
    if @show_battled_count
      textpos.push([_INTL("战斗次数"), 314, 160, :left, base, shadow])
      textpos.push([$player.pokedex.battled_count(@species).to_s, 452, 200, :right, base, shadow])
    else
      textpos.push([_INTL("高度"), 314, 168, :left, base, shadow])
      textpos.push([_INTL("重量"), 314, 200, :left, base, shadow])
    end
    if $player.owned?(@species)
      # Write the category
      textpos.push([_INTL("{1}宝可梦", species_data.category), 246, 84, :left, base, shadow])
      ...
      # Write the category
      textpos.push([_INTL("？？？宝可梦"), 246, 8, :left, base, shadow])
      # Write the height and weight
      if !@show_battled_count
        if System.user_language[3..4] == "US"   # If the user is in the United States
          textpos.push([_INTL("？？？"), 460, 168, :right, base, shadow])
          textpos.push([_INTL("？？？磅"), 494, 200, :right, base, shadow])
        else
          textpos.push([_INTL("？？？米"), 470, 168, :right, base, shadow])
          textpos.push([_INTL("？？？千克"), 482, 200, :right, base, shadow])
        end
      end
    end
    # Draw all text
  ...
  def drawPageArea
    ...
    if points.length == 0
      pbDrawImagePositions(
        overlay,
        [["Graphics/UI/Pokedex/overlay_areanone", 108, 180]]
      )
      textpos.push([_INTL("栖息地未知"), Graphics.width / 2, (Graphics.height / 2), :center, base, shadow])
    end
    textpos.push([@mapdata.name, 414, 50, :center, base, shadow])
    textpos.push([_INTL("{1}的栖息地", GameData::Species.get(@species).name),
                  Graphics.width / 2, 362, :center, base, shadow])
    pbDrawTextPositions(overlay, textpos)
  end
