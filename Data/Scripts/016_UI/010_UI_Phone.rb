...
class PokemonPhone_Scene
  ...
  def pbRefreshList
    ...
    # Set info text
    infotext = _INTL("已登记") + "<br>"
    infotext += "<r>" + @sprites["list"].commands.length.to_s + "<br>"
    infotext += _INTL("等待重战") + "<r>" + rematch_count.to_s
  ...
  def pbStartScreen
    ...
    loop do
      ...
      when 1   # Delete
        ...
        if pbConfirmMessage(_INTL("你确定要删除{1}吗？", name))
          contact.visible = false
          $PokemonGlobal.phone.sort_contacts
          @scene.pbRefreshList
          pbMessage(_INTL("{1}已从联系人中删除。", name))
          if $PokemonGlobal.phone.contacts.none? { |con| con.visible? }
            pbMessage(_INTL("没有记录电话号码。"))
            break
          end
      ...
      when 2   # Sort Contacts
        case pbMessage(_INTL("你希望怎么排序联系人？"),
                       [_INTL("根据名字"),
                        _INTL("根据训练家类型"),
                        _INTL("特殊联系人优先"),
                        _INTL("取消")], -1, nil, 0)
