...
class Window_PokemonMart < Window_DrawableCommand
  ...
  def drawItem(index, count, rect)
    ...
    ypos = rect.y
    if index == count - 1
      textpos.push([_INTL("取消"), rect.x, ypos + 2, :left, self.baseColor, self.shadowColor])
    else
      item = @stock[index]
...
class PokemonMart_Scene
  ...
  def pbRefresh
    if @subscene
      ...
      @sprites["itemtextwindow"].text =
        (itemwindow.item) ? @adapter.getDescription(itemwindow.item) : _INTL("取消购买")
      @sprites["qtywindow"].visible = !itemwindow.item.nil?
      @sprites["qtywindow"].text    = _INTL("包里有：<r>{1}", @adapter.getQuantity(itemwindow.item))
      @sprites["qtywindow"].y       = Graphics.height - 102 - @sprites["qtywindow"].height
      itemwindow.refresh
    end
    @sprites["moneywindow"].text = _INTL("金钱：\n{1}", @adapter.getMoneyString)
  end
  ...
  def pbStartBuyOrSellScene(buying, stock, adapter)
    ...
    @sprites["qtywindow"].text = _INTL("包里有：<r>{1}", @adapter.getQuantity(@sprites["itemwindow"].item))
    @sprites["qtywindow"].y    = Graphics.height - 102 - @sprites["qtywindow"].height
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getMoney < price
        pbDisplayPaused(_INTL("您没有足够的钱。"))
        next
      end
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("购买{1}需要${2}，确定吗？",
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("您想要多少{1}？", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("购买{1}个{2}需要${3}，确定吗？",
                                   quantity, itemnameplural, price.to_s_formatted))
        elsif quantity > 0
          next if !pbConfirm(_INTL("购买1个{2}需要${3}，确定吗？",
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("给您！谢谢惠顾！")) { pbSEPlay("Mart buy item") }
        if quantity >= 10 && GameData::Item.exists?(:PREMIERBALL)
          if Settings::MORE_BONUS_PREMIER_BALLS && GameData::Item.get(item).is_poke_ball?
            ...
            pbDisplayPaused(_INTL("谢谢惠顾，附赠您{1}个{2}！", premier_balls_added, ball_name))
            ...
            if @adapter.addItem(:PREMIERBALL)
              ...
              pbDisplayPaused(_INTL("谢谢惠顾，附赠您1个{1}！", ball_name))
            end
          end
        end
      else
        added.times do
          if !@adapter.removeItem(item)
            raise _INTL("删除存储的道具失败")
          end
        end
        pbDisplayPaused(_INTL("包里已经没有位置存放了。"))
      end
    end
    @scene.pbEndBuyScene
  end
  ...
  def pbSellScreen
    ...
    loop do
      ...
      if !@adapter.canSell?(item)
        pbDisplayPaused(_INTL("抱歉，我不能买下{1}。", itemnameplural))
        next
      end
      ...
      if qty > 1
        qty = @scene.pbChooseNumber(
          _INTL("您想要卖多少{1}呢？", itemnameplural), item, qty
        )
      end
      if qty == 0
        @scene.pbHideMoney
      ...
      if pbConfirm(_INTL("我可以支付您${1}，\n可以接受吗？", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("你交出了{1}，获得了${2}。",
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  cmdSell = -1
  cmdQuit = -1
  commands[cmdBuy = commands.length]  = _INTL("购买道具")
  commands[cmdSell = commands.length] = _INTL("售卖道具") if !cantsell
  commands[cmdQuit = commands.length] = _INTL("没有，谢谢")
  cmd = pbMessage(speech || _INTL("欢迎！请问您有什么需要帮助的吗？"), commands, cmdQuit + 1)
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      ...
      pbMessage(_INTL("欢迎下次光临！"))
      break
    end
    cmd = pbMessage(_INTL("还有什么我可以为您做的吗？"), commands, cmdQuit + 1)
  end
