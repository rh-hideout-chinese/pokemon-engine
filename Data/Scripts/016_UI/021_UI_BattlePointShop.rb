...
class BattlePointShop_Scene
  ...
  def pbRefresh
    ...
    @sprites["battlepointwindow"].text = _INTL("对战点数：\n<r>{1}", @adapter.getBPString)
  end
  ...
  def pbBuyScreen
    ...
    loop do
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("您没有足够的对战点数。"))
        next
      end
      if GameData::Item.get(item).is_important?
        next if !pbConfirm(_INTL("购买{1}需要{2}点BP，确定吗？",
        ...
        quantity = @scene.pbChooseNumber(
          _INTL("您想要多少{1}呢？", itemnameplural), item, maxafford
        )
        next if quantity == 0
        price *= quantity
        if quantity > 1
          next if !pbConfirm(_INTL("购买{1}个{2}需要{3}点BP，确定吗？",
      ...
      if @adapter.getBP < price
        pbDisplayPaused(_INTL("抱歉，您没有足够的对战点数。"))
        next
      end
      added = 0
      quantity.times do
...
def pbBattlePointShop(stock, speech = nil)
  ...
  if speech.nil?
    pbMessage(_INTL("欢迎来到点数兑换处！"))
    pbMessage(_INTL("您可以在这用BP换一些好东西。"))
  else
    pbMessage(speech)
  end
  ...
  screen.pbBuyScreen
  pbMessage(_INTL("感谢您的来访。"))
  pbMessage(_INTL("当您有了更多BP时，请一定要再次来访。"))
