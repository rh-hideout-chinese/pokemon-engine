...
class PokemonBoxPartySprite < Sprite
  ...
  def refresh
    ...
    pbDrawTextPositions(
      self.bitmap,
      [[_INTL("返回"), 86, 248, :center, Color.new(248, 248, 248), Color.new(80, 80, 80), :outline]]
    )
...
class PokemonStorageScene
  ...
  def pbMark(selected, heldpoke)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          [_INTL("确认"), 402, 216, :center, base, shadow, :outline],
          [_INTL("取消"), 402, 280, :center, base, shadow, :outline]
        ]
  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    pbDrawTextPositions(
      overlay,
      [[_INTL("队伍：{1}", (@storage.party.length rescue 0)), 270, 334, :center, buttonbase, buttonshadow, :outline],
       [_INTL("退出"), 446, 334, :center, buttonbase, buttonshadow, :outline]]
    )
    pokemon = nil
    if @screen.pbHeldPokemon
    ...
    if !pokemon.egg?
      ...
      if pokemon.ability
        ...
        textstrings.push([_INTL("无特性"), 86, 312, :center, nonbase, nonshadow])
      end
      if pokemon.item
        textstrings.push([pokemon.item.name, 86, 348, :center, base, shadow])
      else
        textstrings.push([_INTL("无道具"), 86, 348, :center, nonbase, nonshadow])
      end
...
class PokemonStorageScreen
  ...
  def pbStartScreen(command)
    ...
    when 0   # Organise
      ...
      loop do
        ...
        if selected.nil?
          if pbHeldPokemon
            pbDisplay(_INTL("正拿着宝可梦！"))
            next
          end
          next if pbConfirm(_INTL("继续操作吗？"))
          break
          ...
          if @scene.quickswap
            ...
            cmdDebug    = -1
            if heldpoke
              helptext = _INTL("已选择{1}。", heldpoke.name)
              commands[cmdMove = commands.length] = (pokemon) ? _INTL("交换") : _INTL("放置")
            elsif pokemon
              helptext = _INTL("已选择{1}。", pokemon.name)
              commands[cmdMove = commands.length] = _INTL("移动")
            end
    ...
    when 1   # Withdraw
      ...
      loop do
        ...
        if selected.nil?
          ...
          pokemon = @storage[selected[0], selected[1]]
          next if !pokemon
          command = pbShowCommands(_INTL("已选择{1}。", pokemon.name),
                                   [_INTL("取出"),
                                    _INTL("信息"),
                                    _INTL("标记"),
                                    _INTL("放生"),
  ...
  def pbConfirm(str)
    return pbShowCommands(str, [_INTL("是"), _INTL("否")]) == 0
  end
  ...
  def pbWithdraw(selected, heldpoke)
    box = selected[0]
    index = selected[1]
    raise _INTL("不能取出宝可梦……") if box == -1
    if @storage.party_full?
      pbDisplay(_INTL("队伍已经满了！"))
      return false
    end
  ...
  def pbStore(selected, heldpoke)
    box = selected[0]
    index = selected[1]
    raise _INTL("不能储存宝可梦……") if box != -1
    if pbAbleCount <= 1 && pbAble?(@storage[box, index]) && !heldpoke
      pbPlayBuzzerSE
      pbDisplay(_INTL("这是最后的宝可梦了！"))
    elsif heldpoke&.mail
      pbDisplay(_INTL("请先取出宝可梦的邮件。"))
    elsif !heldpoke && @storage[box, index].mail
      pbDisplay(_INTL("请先取出宝可梦的邮件。"))
    elsif heldpoke&.cannot_store
      pbDisplay(_INTL("{1}拒绝进入电脑！", heldpoke.name))
    elsif !heldpoke && @storage[box, index].cannot_store
      pbDisplay(_INTL("{1}拒绝进入电脑！", @storage[box, index].name))
    else
      loop do
        destbox = @scene.pbChooseBox(_INTL("储存到哪个盒子？"))
        if destbox >= 0
          firstfree = @storage.pbFirstFreePos(destbox)
          if firstfree < 0
            pbDisplay(_INTL("盒子已经满了……"))
            next
          end
  ...
  def pbPlace(selected)
    ...
    if @storage[box, index]
      raise _INTL("位置{1}，{2}不为空……", box, index)
    elsif box != -1
      if index >= @storage.maxPokemon(box)
        ...
        pbDisplay(_INTL("{1}拒绝进入电脑！", @heldpkmn.name))
        return
      end
    end
  ...
  def pbSwap(selected)
    ...
    index = selected[1]
    if !@storage[box, index]
      raise _INTL("位置{1}，{2}为空……", box, index)
    end
  ...
  def pbRelease(selected, heldpoke)
    ...
    if pokemon.egg?
      ...
      pbDisplay(_INTL("{1}不想离开你！", pokemon.name))
      return false
    end
    ...
    command = pbShowCommands(_INTL("要将宝可梦放生？"), [_INTL("否"), _INTL("是")])
    if command == 1
  ...
  def pbItem(selected, heldpoke)
    ...
    if pokemon.item
      ...
      if pbConfirm(_INTL("拿走{1}？", itemname))
        if $bag.add(pokemon.item)
          pbDisplay(_INTL("拿走了{1}。", itemname))
          pokemon.item = nil
          @scene.pbHardRefresh
        else
          pbDisplay(_INTL("不能存储{1}。", itemname))
        end
      end
    else
      ...
      if item
        ...
        pokemon.item = item
        $bag.remove(item)
        pbDisplay(_INTL("{1}现在被携带着。", itemname))
        @scene.pbHardRefresh
      end
    end
  end
  ...
  def pbBoxCommands
    ...
    command = pbShowCommands(_INTL("想要做什么？"), commands)
    case command
    when 0
      destbox = @scene.pbChooseBox(_INTL("跳转到哪个盒子？"))
    ...
    when 1
      ...
      wpaper = pbShowCommands(_INTL("选择壁纸。"), papers[0], index)
      @scene.pbChangeBackground(papers[1][wpaper]) if wpaper >= 0
    when 2
      @scene.pbBoxName(_INTL("盒子的名字是？"), 0, 12)
    end
  end
  ...
  def pbChoosePokemon(_party = nil)
    ...
    loop do
      ...
      if selected.nil?
        ...
        commands[2] = _INTL("存放") if selected[0] == -1
        helptext = _INTL("已选择{1}。", pokemon.name)
        command = pbShowCommands(helptext, commands)
        case command
        when 0   # Select
