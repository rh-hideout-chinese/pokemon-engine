...
#    end
  ...
  def pbPlace(pkmn, position)
    ...
    if pkmn.egg?
      @scene.pbDisplay(_INTL("不能把蛋放在这里。"))
      return false
    end
    if position == 0
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("只有黑暗宝可梦才能放在这里。"))
        return false
      end
    elsif position >= 1
      if pkmn.shadowPokemon?
        @scene.pbDisplay(_INTL("不能在这放置黑暗宝可梦。"))
        return false
      else
        oldpkmn = PurifyChamberHelper.pbGetPokemon(@chamber, position)
        if oldpkmn
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set) == 0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
        _INTL("{1}准备好解开束缚了，但是仪式至少还要再有一只正常的宝可梦才能开始。",
              pkmn.name)
      )
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      when 0   # Place Pokemon in the set
        ...
        if curpkmn || heldpkmn
          commands = [_INTL("移动"), _INTL("信息"), _INTL("取回")]
          if curpkmn && heldpkmn
            commands[0] = _INTL("替换")
          elsif heldpkmn
            commands[0] = _INTL("场所")
          ...
          if !heldpkmn && curpkmn
            commands[cmdReplace = commands.length] = _INTL("更换")
          end
          commands.push(_INTL("取消"))
          choice = @scene.pbShowCommands(
            _INTL("{1}应该要？", heldpkmn ? heldpkmn.name : curpkmn.name),
            commands
          )
          if choice == 0
            if heldpkmn
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.shadowPokemon?) == (curpkmn.shadowPokemon?)
                  ...
                  @scene.pbDisplay(_INTL("这个宝可梦不能放在那里。"))
                end
              end
            end
          end
      ...
      when 2   # Choose a Pokemon
        ...
        if heldpkmn
          @scene.pbDisplay(_INTL("正拿着宝可梦！"))
        else
          break if !@scene.pbConfirm(_INTL("继续编辑此舞台？"))
        end
      end
    end
    if pbCheckPurify
      @scene.pbDisplay(_INTL("有宝可梦准备好解开心灵的束缚！") + "\1")
  ...
  def pbDoPurify
    ...
    purifiables.length.times do |i|
      ...
      if (i + 1) != purifiables.length
        @scene.pbDisplay(_INTL("还有宝可梦准备好解开心灵的束缚！") + "\1")
        if !@scene.pbConfirm(_INTL("你想要交换舞台吗？"))
          @scene.pbCloseSet
          break
        end
      end
  ...
  def pbStartPurify
    ...
    loop do
      ...
      if set < 0
        break if !@scene.pbConfirm(_INTL("继续查看全息图？"))
      else
        chamber.currentSet = set
        cmd = @scene.pbShowCommands(_INTL("想要做什么？"),
                                    [_INTL("编辑"), _INTL("交换"), _INTL("取消")])
        case cmd
        when 0   # edit
...
class PurifyChamberSetView < Sprite
  ...
  def refresh
    ...
    if pkmn
      ...
      textpos.push([_INTL("强度"), 2 + (@info.bitmap.width / 2), 30, :left,
...
MenuHandlers.add(:pc_menu, :purify_chamber, {
  ...
  "effect"    => proc { |menu|
    pbMessage("\\se[PC access]" + _INTL("进入了净化室。"))
    pbPurifyChamber
    next false
  }
})
