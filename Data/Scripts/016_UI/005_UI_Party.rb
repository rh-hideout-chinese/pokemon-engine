...
class PokemonPartyCancelSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("取消"), 398, 328, false, viewport)
  end
end
...
class PokemonPartyConfirmSprite < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("确认"), 398, 308, true, viewport)
  end
end
...
class PokemonPartyCancelSprite2 < PokemonPartyConfirmCancelSprite
  def initialize(viewport = nil)
    super(_INTL("取消"), 398, 346, true, viewport)
  end
end
...
class PokemonPartyPanel < Sprite
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.name, 96, 26, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_level
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.level.to_s, 42, 74, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    pbSetSystemFont(@overlaysprite.bitmap)
  end
  ...
  def draw_hp
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[hp_text, 224, 70, :right, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
    # HP bar
    if @pokemon.able?
...
class PokemonParty_Scene
  def pbStartScene(party, starthelptext, annotations = nil, multiselect = false, can_access_storage = false)
    ...
    @sprites["storagetext"] = Window_UnformattedTextPokemon.new(
      @can_access_storage ? _INTL("[Special]：开启存储") : ""
    )
...
class PokemonPartyScreen
  ...
  def pbPokemonGiveMailScreen(mailIndex)
    ...
    if pkmnid >= 0
      ...
      if pkmn.hasItem? || pkmn.mail
        pbDisplay(_INTL("宝可梦已经携带道具了，不能携带邮件。"))
      elsif pkmn.egg?
        pbDisplay(_INTL("蛋不能携带邮件。"))
      else
  ...
  def pbPokemonMultipleEntryScreenEx(ruleset)
    annot = []
    statuses = []
    ordinals = [_INTL("不符合"), _INTL("未参加"), _INTL("禁止")]
    ...
    loop do
      ...
      if (statuses[pkmnid] || 0) == 1
        ...
        commands[cmdNoEntry = commands.length] = _INTL("不参加")
      end
      pkmn = @party[pkmnid]
      commands[cmdSummary = commands.length]   = _INTL("查看信息")
      commands[commands.length]                = _INTL("取消")
      command = @scene.pbShowCommands(_INTL("对{1}做什么？", pkmn.name), commands) if pkmn
      if cmdEntry >= 0 && command == cmdEntry
        ...
        @scene.pbSummary(pkmnid) do
          @scene.pbSetHelpText((@party.length > 1) ? _INTL("选择一只宝可梦。") : _INTL("选择宝可梦或取消。"))
        end
      end
    end
    @scene.pbEndScene
  ...
  def pbChooseAblePokemon(ableProc, allowIneligible = false)
    ...
    @scene.pbStartScene(
      @party,
      (@party.length > 1) ? _INTL("选择一只宝可梦。") : _INTL("选择宝可梦或取消。"),
      annot
    )
    loop do
      @scene.pbSetHelpText(
  ...
  def pbPokemonScreen
    ...
    loop do
      ...
      choice = @scene.pbShowCommands(_INTL("对{1}做什么？", pkmn.name), command_list)
      ...
      when Integer   # Hidden move's index
        ...
        if [:MILKDRINK, :SOFTBOILED].include?(move.id)
          ...
          @scene.pbSetHelpText(_INTL("对哪个宝可梦使用？"))
          ...
          loop do
            ...
            if party_idx == old_party_idx
              ...
              @scene.pbDisplay(_INTL("{1}回复了{2}点HP。", newpkmn.name, hpgain))
              pbRefresh
            end
          ...
          if pbConfirmUseHiddenMove(pkmn, move.id)
            ...
            if move.id == :FLY
              ...
              @scene.pbStartScene(
                @party, (@party.length > 1) ? _INTL("选择一只宝可梦。") : _INTL("选择宝可梦或取消。")
              )
              next
            end
            return [pkmn, move.id]
...
MenuHandlers.add(:party_menu, :summary, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    screen.scene.pbSummary(party_idx) do
      screen.scene.pbSetHelpText((party.length > 1) ? _INTL("选择一只宝可梦。") : _INTL("选择宝可梦或取消。"))
    end
  }
})
...
MenuHandlers.add(:party_menu, :mail, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    pkmn = party[party_idx]
    command = screen.scene.pbShowCommands(_INTL("对邮件做什么？"),
                                          [_INTL("阅读"), _INTL("取回"), _INTL("取消")])
    case command
    when 0   # Read
      pbFadeOutIn do
...
MenuHandlers.add(:party_menu, :item, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    command_list.push(_INTL("取消"))
    # Choose a menu option
    choice = screen.scene.pbShowCommands(_INTL("对道具做什么？"), command_list)
    next if choice < 0 || choice >= commands.length
...
MenuHandlers.add(:party_menu_item, :move, {
  ...
  "effect"    => proc { |screen, party, party_idx|
    ...
    screen.scene.pbSetHelpText(_INTL("将{1}移动到哪里？", itemname))
    old_party_idx = party_idx
    moved = false
    loop do
      ...
      if newpkmn.egg?
        screen.pbDisplay(_INTL("蛋不能携带道具。"))
        next
        ...
        screen.pbRefresh
        screen.pbDisplay(_INTL("{1}已交给{2}携带。", newpkmn.name, portionitemname))
        moved = true
        break
      elsif newpkmn.item.is_mail?
        screen.pbDisplay(_INTL("给予道具前，必须先拿回{1}身上的邮件。", newpkmn.name))
        next
      end
      ...
      if newitemname.starts_with_vowel?
        screen.pbDisplay(_INTL("{1}已经携带{2}了。", newpkmn.name, newitemname) + "\1")
      else
        screen.pbDisplay(_INTL("{1}已经携带{2}了。", newpkmn.name, newitemname) + "\1")
      end
      next if !screen.pbConfirm(_INTL("想要交换这两个道具吗？"))
      newpkmn.item = item
      pkmn.item = newitem
      screen.scene.pbClearSwitching
      screen.pbRefresh
      screen.pbDisplay(_INTL("{1}已交给{2}携带。", newpkmn.name, portionitemname) + "\1")
      screen.pbDisplay(_INTL("{1}已交给{2}携带。", pkmn.name, newitemname))
      moved = true
      break
    end
...
def pbChoosePokemon(variableNumber, nameVarNumber, ableProc = nil, allowIneligible = false)
  ...
  pbFadeOutIn do
    ...
    if ableProc
      ...
      screen.pbStartScene(_INTL("选择一只宝可梦。"), false)
