...
class PokemonSummary_Scene
  ...
  def pbConfirm(text)
    ...
    @sprites["messagebox"].text    = text
    @sprites["messagebox"].visible = true
    using(cmdwindow = Window_CommandPokemon.new([_INTL("是"), _INTL("否")])) do
  ...
  def drawPage(page)
    ...
    textpos = [
      ...
      [@pokemon.level.to_s, 46, 100, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)],
    ...
    if @pokemon.male?
      textpos.push([_INTL("♂"), 172, 68, :left, Color.new(24, 112, 216), Color.new(136, 168, 208)])
    elsif @pokemon.female?
      textpos.push([_INTL("♀"), 172, 68, :left, Color.new(248, 56, 32), Color.new(224, 152, 144)])
    end
    # Draw all text
  ...
  def drawPageOne
    ...
    # Write various bits of text
    textpos = [
      [_INTL("图鉴信息"), 238, 86, :left, base, shadow],
      [_INTL("名字"), 238, 118, :left, base, shadow],
      [@pokemon.speciesName, 435, 118, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)],
      [_INTL("属性"), 238, 150, :left, base, shadow],
      [_INTL("初训家"), 238, 182, :left, base, shadow],
      [_INTL("ID No."), 238, 214, :left, base, shadow]
    ]
    # Write the Regional/National Dex number
    dexnum = @nationalDexList.index(@pokemon.species_data.species)
    dexnumshift = false
    ...
    if dexnum <= 0
      textpos.push(["???", 435, 90, :center, dexNumBase, dexNumShadow])
    else
      dexnum -= 1 if dexnumshift
      textpos.push([sprintf("%03d", dexnum), 435, 90, :center, dexNumBase, dexNumShadow])
    end
    # Write Original Trainer's name and ID number
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("租借"), 435, 182, :center, Color.new(64, 64, 64), Color.new(176, 176, 176)])
    ...
    if @pokemon.shadowPokemon?
      textpos.push([_INTL("净化计"), 238, 242, :left, base, shadow])
      black_text_tag = shadowc3tag(BLACK_TEXT_BASE, BLACK_TEXT_SHADOW)
      heartmessage = [_INTL("心门打开了！解开最后的束缚吧！"),
                      _INTL("心门几乎打开了。"),
                      _INTL("心门快要打开了。"),
                      _INTL("心门越开越大。"),
                      _INTL("心门正在打开。"),
      ...
      textpos.push([_INTL("经验值"), 238, 246, :left, base, shadow])
      textpos.push([@pokemon.exp.to_s_formatted, 488, 278, :right, Color.new(64, 64, 64), Color.new(176, 176, 176)])
      textpos.push([_INTL("距离下一等级"), 238, 310, :left, base, shadow])
  ...
  def drawPageOneEgg
    ...
    # Write various bits of text
    textpos = [
      [_INTL("简介"), 26, 18, :left, base, shadow],
      [@pokemon.name, 46, 68, :left, base, shadow],
      [_INTL("道具"), 66, 324, :left, base, shadow]
    ]
    # Write the held item's name
    if @pokemon.hasItem?
      ...
      textpos.push([_INTL("无"), 16, 362, :left, Color.new(192, 200, 208), Color.new(208, 216, 224)])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    ...
    if mapname && mapname != ""
      ...
      memo += black_text_tag + _INTL("在{1}获得的神秘的宝可梦蛋。", mapname) + "\n"
    else
      memo += black_text_tag + _INTL("神秘的宝可梦蛋。") + "\n"
    end
    memo += "\n"   # Empty line
    ...
    # Draw all text
    drawFormattedTextEx(overlay, 232, 90, 268, memo)
    # Draw the Pokémon's markings
    drawMarkings(overlay, 84, 292)
  end
  ...
  def drawPageTwo
    ...
    mettext = [
      _INTL("在{1}级时相遇。", @pokemon.obtain_level),
      _INTL("获得了蛋"),
      _INTL("在{1}级时交换而得。", @pokemon.obtain_level),
      "",
      _INTL("Lv.{1}命中注定般地遇见了。", @pokemon.obtain_level)
    ...
    # Write all text
    drawFormattedTextEx(overlay, 236, 86, 272, memo)
  end
  ...
  def drawPageThree
    ...
    if ability
      ...
      drawTextEx(overlay, 224, 323, 288, 2, ability.description, Color.new(64, 64, 64), Color.new(176, 176, 176))
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw HP bar
  ...
  def drawPageFour
    ...
    Pokemon::MAX_MOVES.times do |i|
      ...
      if move
        ...
        if move.total_pp > 0
          textpos.push([_INTL("PP"), 346, yPos + 34, :left, moveBase, moveShadow])
          ppfraction = 0
          if move.pp == 0
            ppfraction = 3
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("招式"), 26, 22, :left, base, shadow],
      [_INTL("分类"), 20, 128, :left, base, shadow],
      [_INTL("威力"), 20, 160, :left, base, shadow],
      [_INTL("命中率"), 20, 192, :left, base, shadow]
    ]
    imagepos = []
    # Write move names, types and PP amounts for each known move
    yPos = 106
    ...
    limit.times do |i|
      ...
      if move
        type_number = GameData::Type.get(move.display_type(@pokemon)).icon_position
        imagepos.push([_INTL("Graphics/UI/types"), 248, yPos - 2, 0, type_number * 28, 64, 28])
    ...
    @pokemon.types.each_with_index do |type, i|
      type_number = GameData::Type.get(type).icon_position
      type_rect = Rect.new(0, type_number * 28, 60, 28)
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    else        textpos.push([selected_move.display_damage(@pokemon).to_s, 216, 164, :right, base, shadow])
    end
    if selected_move.display_accuracy(@pokemon) == 0
      textpos.push(["---", 216, 192, :right, base, shadow])
    else
      textpos.push(["#{selected_move.display_accuracy(@pokemon)}%", 210 + overlay.text_size("%").width, 194, :right, base, shadow])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw selected move's damage category icon
    imagepos = [["Graphics/UI/category", 166, 120, 0, selected_move.display_category(@pokemon) * 28, 64, 28]]
    pbDrawImagePositions(overlay, imagepos)
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
      [_INTL("奖章数量："), 234, 338, :left, Color.new(64, 64, 64), Color.new(176, 176, 176)],
  ...
  def pbMarking(pokemon)
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          [_INTL("标记{1}", pokemon.name), 366, 102, :center, base, shadow],
          [_INTL("确认"), 366, 254, :center, base, shadow],
          [_INTL("取消"), 366, 304, :center, base, shadow]
        ]
