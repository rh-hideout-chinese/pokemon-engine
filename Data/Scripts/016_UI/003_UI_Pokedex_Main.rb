...
class Window_Pokedex < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    pbDrawShadowText(self.contents, rect.x + 26, rect.y + 10, rect.width, rect.height,
                     num_text, self.baseColor, self.shadowColor)
    pbDrawShadowText(self.contents, rect.x + 76, rect.y + 10, rect.width, rect.height,
...
#    end
  ...
  def pbRefreshDexSearch(params, _index)
    ...
    # Write various bits of text
    textpos = [
      [_INTL("搜索模式"), Graphics.width / 2, 10, :center, base, shadow],
      [_INTL("顺序"), 136, 64, :center, base, shadow],
      [_INTL("名字"), 58, 122, :center, base, shadow],
      [_INTL("属性"), 58, 174, :center, base, shadow],
      [_INTL("高度"), 58, 226, :center, base, shadow],
      [_INTL("重量"), 58, 278, :center, base, shadow],
      [_INTL("颜色"), 326, 122, :center, base, shadow],
      [_INTL("形状"), 454, 174, :center, base, shadow],
      [_INTL("复位"), 80, 346, :center, base, shadow, 1],
      [_INTL("开始"), Graphics.width / 2, 346, :center, base, shadow, :outline],
      [_INTL("取消"), Graphics.width - 80, 346, :center, base, shadow, :outline]
    ]
  ...
  def pbRefreshDexSearchParam(mode, cmds, sel, _index)
    ...
    textpos = [
      [_INTL("搜索模式"), Graphics.width / 2, 10, :center, base, shadow],
      [_INTL("确认"), 80, 346, :center, base, shadow, :outline],
      [_INTL("取消"), Graphics.width - 80, 346, :center, base, shadow, :outline]
    ]
    title = [_INTL("顺序"), _INTL("名字"), _INTL("属性"), _INTL("高度"),
             _INTL("重量"), _INTL("颜色"), _INTL("形状")][mode]
  ...
  def pbDexSearch
    ...
    @orderCommands = []
    @orderCommands[MODENUMERICAL] = _INTL("数值")
    @orderCommands[MODEATOZ]      = _INTL("A到Z")
