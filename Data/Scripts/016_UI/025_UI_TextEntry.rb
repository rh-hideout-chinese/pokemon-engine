...
class Window_CharacterEntry < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    if index == @charset.length # -1
      ...
      pbDrawShadowText(self.contents, rect.x, rect.y, rect.width, rect.height, _INTL("确认"),
                       self.baseColor, self.shadowColor)
    else
...
class PokemonEntryScene2
  @@Characters = [
    ...
    [("ÀÁÂÄÃàáâäã Ææ" + "ÈÉÊË èéêë  Çç" + "ÌÍÎÏ ìíîï  Œœ" + "ÒÓÔÖÕòóôöõ Ññ" + "ÙÚÛÜ ùúûü  Ýý").scan(/./), _INTL("拼音")],
