...
class PokemonTrainerCard_Scene
  ...
  def pbStartScene
    ...
    @sprites["trainer"] = IconSprite.new(415, 255, @viewport)
  ...
  def pbDrawTrainerCardFront
    ...
    hour = totalsec / 60 / 60
    min = totalsec / 60 % 60
    time = (hour > 0) ? _INTL("{1}时{2}分", hour, min) : _INTL("{1}分", min)
    $PokemonGlobal.startTime = Time.now if !$PokemonGlobal.startTime
    starttime = _INTL("{3}年{1}{2}日",
                      pbGetAbbrevMonthName($PokemonGlobal.startTime.mon),
                      $PokemonGlobal.startTime.day,
                      $PokemonGlobal.startTime.year)
    textPositions = [
      [_INTL("名称"), 34, 70, :left, baseColor, shadowColor],
