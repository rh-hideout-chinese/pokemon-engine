...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    textpos = [
      [_INTL("教什么招式？"), 16, 14, :left, Color.new(88, 88, 80), Color.new(168, 184, 184)]
    ]
    imagepos = []
    yPos = 88
    ...
    textpos.push([_INTL("分类"), 272, 120, :left, Color.new(248, 248, 248), Color.black])
    textpos.push([_INTL("威力"), 272, 152, :left, Color.new(248, 248, 248), Color.black])
    textpos.push([power <= 1 ? power == 1 ? "???" : "---" : power.to_s, 468, 152, :center,
                  Color.new(64, 64, 64), Color.new(176, 176, 176)])
    textpos.push([_INTL("命中"), 272, 184, :left, Color.new(248, 248, 248), Color.black])
...
class MoveRelearnerScreen
  ...
  def pbStartScreen(pkmn)
    ...
    loop do
      ...
      if move
        if @scene.pbConfirm(_INTL("教学{1}？", GameData::Move.get(move).name))
      ...
      elsif @scene.pbConfirm(_INTL("放弃教学{1}新招式？", pkmn.name))
        @scene.pbEndScene
        return false
      end
    end
