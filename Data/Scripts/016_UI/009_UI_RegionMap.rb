...
class PokemonRegionMap_Scene
  ...
  def pbChangeMapLocation(x, y)
    ...
    currentname = pbMessageFreeText(_INTL("设置此地的名称"), currentname, false, 250) { pbUpdate }
    if currentname
      if currentobj
  ...
  def refresh_fly_screen
    ...
    text = (@mode == 0) ? _INTL("ACTION：飞翔") : _INTL("ACTION：取消飞翔")
    pbDrawTextPositions(
      @sprites["help"].bitmap,
  ...
  def pbMapScene
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        if @editor && @changed
          pbSaveMapData if pbConfirmMessage(_INTL("保存修改？")) { pbUpdate }
          break if pbConfirmMessage(_INTL("退出地图？")) { pbUpdate }
        else
          break
        end
        ...
        if healspot && ($PokemonGlobal.visitedMaps[healspot[0]] ||
          ...
          return healspot if pbConfirmMessage(_INTL("您想飞到{1}吗？", name)) { pbUpdate }
        end
