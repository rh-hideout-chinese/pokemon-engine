...
class PokemonTrade_Scene
  ...
  def pbTrade
    ...
    pbScene1
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("{3}选择{4}\n交换\n{1}的{2}", @trader1, speciesname1, @trader2, speciesname2) + "\1") { pbUpdate }
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("{1}向{2}告别了。", @trader2, speciesname2)) { pbUpdate }
    pbScene2
    pbBGMStop
    ...
    pbMessageDisplay(@sprites["msgwindow"],
                     _INTL("请照顾好{1}！", speciesname2)) { pbUpdate }
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
      ...
      pbMessageDisplay(@sprites["msgwindow"],
                       _INTL("{1}的资料被添加到宝可梦图鉴里了！", speciesname2)) { pbUpdate }
