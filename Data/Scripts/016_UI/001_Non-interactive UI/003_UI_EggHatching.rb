...
class PokemonEggHatch_Scene
  ...
  def pbMain
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
      ...
      pbMessage(_INTL("{1}的资料被添加到宝可梦图鉴里了！", @pokemon.name)) { update }
    ...
    if $PokemonSystem.givenicknames == 0 &&
      ...
      nickname = pbEnterPokemonName(_INTL("{1}的昵称是？", @pokemon.name),
...
def pbHatch(pokemon)
  ...
  if !pbHatchAnimation(pokemon)
    ...
    pbMessage(_INTL("……") + "\1")
    pbMessage(_INTL("… …… ………") + "\1")
    pbMessage(_INTL("蛋孵化成{1}了！", speciesname))
    was_owned = $player.owned?(pokemon.species)
    $player.pokedex.register(pokemon)
    $player.pokedex.set_owned(pokemon.species)
    $player.pokedex.set_seen_egg(pokemon.species)
    ...
    if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && !was_owned &&
       $player.has_pokedex && $player.pokedex.species_in_unlocked_dex?(pokemon.species)
      pbMessage(_INTL("{1}的资料被添加到宝可梦图鉴里了！", speciesname))
      $player.pokedex.register_last_seen(pokemon)
      pbFadeOutIn do
        scene = PokemonPokedexInfo_Scene.new
        screen = PokemonPokedexInfoScreen.new(scene)
    ...
    if $PokemonSystem.givenicknames == 0 &&
      ...
      nickname = pbEnterPokemonName(_INTL("{1}的昵称是？", speciesname),
