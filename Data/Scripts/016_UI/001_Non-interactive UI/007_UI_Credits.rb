  ...
  def get_text
    ...
    if PluginManager.plugins.length > 0
      ...
      PluginManager.plugins.each do |plugin|
        ...
        ret.push(_INTL("“{1}”V.{2} 来自：", plugin, PluginManager.version(plugin)))
        add_names_to_credits(ret, pcred)
      end
    end
    # Add Essentials credits
    ret.push("", "", "")
    ret.push(_INTL("“Pokémon Essentials” 创建者："))
    ...
    ret.push("")
    ret.push(_INTL("“mkxp-z” 来自："))
    add_names_to_credits(ret, [
      "Roza",
      _INTL("基于Ancurio等人的“mkxp”。")
    ])
    ret.push(_INTL("“RPG Maker XP”来自:"))
