...
class HallOfFame_Scene
  ...
  def writeTrainerData
    ...
    lefttext = _INTL("名称<r>{1}", $player.name) + "<br>"
    lefttext += _INTL("ID No.<r>{1}", pubid) + "<br>"
    if hour > 0
      lefttext += _INTL("时长<r>{1}时 {2}分", hour, min) + "<br>"
    else
      lefttext += _INTL("时长<r>{1}分", min) + "<br>"
    end
  ...
  def writePokemonData(pokemon, hallNumber = -1)
    ...
    if hallNumber > -1
      textPositions.push([_INTL("冠军殿堂No."), (Graphics.width / 2) - 104, 6, :left, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR])
  ...
  def writeWelcome
    overlay = @sprites["overlay"].bitmap
    overlay.clear
    pbDrawTextPositions(overlay, [[_INTL("欢迎来到冠军殿堂！"),
...
MenuHandlers.add(:pc_menu, :hall_of_fame, {
  ...
  "effect"    => proc { |menu|
    pbMessage("\\se[PC access]" + _INTL("进入了冠军殿堂。"))
    pbHallOfFamePC
    next false
  }
})
