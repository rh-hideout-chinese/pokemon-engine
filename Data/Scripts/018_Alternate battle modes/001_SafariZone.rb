...
EventHandlers.add(:on_player_step_taken_can_transfer, :safari_game_counter,
  proc { |handled|
    ...
    pbMessage("\\se[Safari Zone end]" + _INTL("广播：叮咚！") + "\1")
    pbMessage(_INTL("广播：游戏结束了！"))
...
def pbSafariBattle(pkmn, level = 1)
  ...
  if pbSafariState.ballcount <= 0
    if decision != 2   # Last Safari Ball was used to catch the wild Pokémon
      pbMessage(_INTL("广播员：你的狩猎球用完了！游戏结束！"))
    end
    pbSafariState.decision = 1
    pbSafariState.pbGoToStart
  end
...
class PokemonPauseMenu
  ...
  def pbShowInfo
    ...
    if Settings::SAFARI_STEPS <= 0
      @scene.pbShowInfo(_INTL("球：{1}", pbSafariState.ballcount))
    else
      @scene.pbShowInfo(_INTL("步数：{1}/{2}\n球：{3}",
...
MenuHandlers.add(:pause_menu, :quit_safari_game, {
  ...
  "effect"    => proc { |menu|
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("你想退出狩猎吗？"))
      menu.pbEndScene
      pbSafariState.decision = 1
      pbSafariState.pbGoToStart
      next true
