...
class PokemonRuleSet
  ...
  def isValid?(team, error = nil)
    if team.length < self.minLength
      error.push(_INTL("选择一只宝可梦")) if error && self.minLength == 1
      error.push(_INTL("至少需要{1}只宝可梦。", self.minLength)) if error && self.minLength > 1
      return false
    elsif team.length > self.maxLength
      error&.push(_INTL("最多让{1}只宝可梦参加。", self.maxLength))
      return false
    end
    team.each do |pkmn|
      next if isPokemonValid?(pkmn)
      if pkmn
        error&.push(_INTL("不能选择{1}。", pkmn.name))
      elsif error
