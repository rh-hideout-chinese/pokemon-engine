...
class BattleSwapScene
  ...
  def pbStartRentScene(rentals)
    ...
    @sprites["title"] = Window_UnformattedTextPokemon.newWithSize(
      _INTL("出租宝可梦"), 0, 0, Graphics.width, 64, @viewport
    )
  ...
  def pbInitSwapScreen
    ...
    @sprites["help"].text = _INTL("选择宝可梦交换。")
  ...
  def pbSwapChosen(_pkmnindex)
    ...
    commands.push(_INTL("交换用宝可梦"))
    commands.push(_INTL("取消"))
    @sprites["help"].text = _INTL("选择宝可梦。")
    @sprites["list"].commands = commands
    @sprites["list"].index = 0
    @mode = 2
  end
    ...
    loop do
      ...
      when 1
        if chosen.include?(index)
          ...
          @scene.pbUpdateChoices(chosen.clone)
          if chosen.length == 3
            if @scene.pbConfirm(_INTL("确定是这些宝可梦吗？"))
              retval = []
      ...
      if pkmn >= 0
        ...
        when 1
          ...
          loop do
            pkmn = @scene.pbChoosePokemon(true)
            if pkmn >= 0
              if @scene.pbConfirm(_INTL("接受这个宝可梦吗？"))
                @scene.pbEndScene
