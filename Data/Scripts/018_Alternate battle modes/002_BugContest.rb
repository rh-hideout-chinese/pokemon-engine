...
class BugContestState
  ...
  def pbJudge
    ...
    raise _INTL("所有捕虫大赛地图都没有遭遇战。") if maps_with_encounters.empty?
    @contestants.each do |cont|
      ...
      raise _INTL("地图{1}没有野生宝可梦，因此无法评判比赛。", enc_data[0]) if !enc
    ...
    raise _INTL("捕虫大赛的选手太少了") if judgearray.length < 3
...
class PokemonPauseMenu
  ...
  def pbShowInfo
    ...
    if pbBugContestState.lastPokemon
      @scene.pbShowInfo(_INTL("捕获：{1}\n等级：{2}\n球：{3}",
                              pbBugContestState.lastPokemon.speciesName,
                              pbBugContestState.lastPokemon.level,
                              pbBugContestState.ballcount))
    else
      @scene.pbShowInfo(_INTL("捕获：无\n球：{1}", pbBugContestState.ballcount))
    end
  end
end
...
MenuHandlers.add(:pause_menu, :quit_bug_contest, {
  ...
  "effect"    => proc { |menu|
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("你想结束比赛吗？"))
      menu.pbEndScene
      pbBugContestState.pbStartJudging
      next true
    end
