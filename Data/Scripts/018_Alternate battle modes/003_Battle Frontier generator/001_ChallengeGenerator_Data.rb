...
def pbGenerateChallenge(rule, tag)
  ...
  iterations.times do |iter|
    ...
    yield(_INTL("生成队伍({2}中的{1})", iter + 1, iterations))
    i = 0
    ...
    yield(_INTL("模拟战斗({2}中的{1})", iter + 1, iterations))
    i = 0
    loop do
      changed = false
...
def pbWriteCup(id, rules)
  ...
  cmd = 0
  if trlists.length == 0
    cmd = pbMessage(_INTL("为挑战生成宝可梦队伍？"),
                    [_INTL("是"), _INTL("否")], 2)
    case cmd
    when 0
      cmd = 2
    ...
    cmd = pbMessage(_INTL("为挑战生成宝可梦队伍？"),
                    [_INTL("否"), _INTL("是，使用现有的队伍"), _INTL("是，使用新的队伍")], 1)
  end
  return if cmd == 0   # No
  case cmd
  when 1   # Yes, use existing
    cmd = pbMessage(_INTL("选择挑战。"), list, -1)
    if cmd >= 0
      pbMessage(_INTL("此挑战将使用{1}中的宝可梦列表。", list[cmd]))
      trlists.length.times do |i|
        tr = trlists[i]
