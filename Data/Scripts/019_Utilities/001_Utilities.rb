...
def getConstantName(mod, value, raise_if_none = true)
  ...
  raise _INTL("{2}中的常量未定义值{1}", value, mod.name) if raise_if_none
  return nil
end
...
def pbMoveTutorChoose(move, movelist = nil, bymachine = false, oneusemachine = false)
  ...
  pbFadeOutIn do
    ...
    screen.pbStartScene(_INTL("教哪个宝可梦？"), false, annot)
    loop do
      chosen = screen.pbChoosePokemon
      break if chosen < 0
      pokemon = $player.party[chosen]
      if pokemon.egg?
        pbMessage(_INTL("蛋不能学习招式。")) { screen.pbUpdate }
      elsif pokemon.shadowPokemon?
        pbMessage(_INTL("黑暗宝可梦不能学习招式。")) { screen.pbUpdate }
      elsif movelist && movelist.none? { |j| j == pokemon.species }
        pbMessage(_INTL("{1}不能学习{2}。", pokemon.name, movename)) { screen.pbUpdate }
