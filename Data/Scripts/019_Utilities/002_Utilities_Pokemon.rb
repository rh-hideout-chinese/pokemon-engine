...
def pbNickname(pkmn)
  ...
  if pbConfirmMessage(_INTL("要给{1}起昵称吗？", species_name))
    pkmn.name = pbEnterPokemonName(_INTL("{1}的昵称是？", species_name),
...
def pbStorePokemon(pkmn)
  if pbBoxesFull?
    pbMessage(_INTL("没有空间给宝可梦了！") + "\1")
    pbMessage(_INTL("盒子都满了，不能接收！"))
    return
  end
  ...
  if $player.party_full?
    ...
    pbMessage(_INTL("{1}已发送至盒子“{2}”！", pkmn.name, box_name))
  else
...
def pbAddPokemon(pkmn, level = 1, see_form = true)
  ...
  species_name = pkmn.speciesName
  pbMessage(_INTL("{1}获得了{2}！", $player.name, species_name) + "\\me[Pkmn get]\\wtnp[80]")
  ...
  if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && see_form && !was_owned &&
    ...
    pbMessage(_INTL("{1}的资料被添加到宝可梦图鉴里了！", species_name))
...
def pbAddForeignPokemon(pkmn, level = 1, owner_name = nil, nickname = nil, owner_gender = 0, see_form = true)
  ...
  pkmn.calc_stats
  if owner_name
    pbMessage(_INTL("{1}从{2}那获得了一只宝可梦。", $player.name, owner_name) + "\\me[Pkmn get]\\wtnp[80]")
  else
    pbMessage(_INTL("{1}获得了一只宝可梦。", $player.name) + "\\me[Pkmn get]\\wtnp[80]")
  end
  was_owned = $player.owned?(pkmn.species)
  $player.pokedex.set_seen(pkmn.species)
  $player.pokedex.set_owned(pkmn.species)
  ...
  if Settings::SHOW_NEW_SPECIES_POKEDEX_ENTRY_MORE_OFTEN && see_form && !was_owned &&
     $player.has_pokedex && $player.pokedex.species_in_unlocked_dex?(pkmn.species)
    pbMessage(_INTL("宝可梦的资料被添加到宝可梦图鉴里了！"))
    $player.pokedex.register_last_seen(pkmn)
    pbFadeOutIn do
      scene = PokemonPokedexInfo_Scene.new
      screen = PokemonPokedexInfoScreen.new(scene)
