...
ItemHandlers::UseFromBag.addIf(:move_machines,
  ...
  proc { |item|
    ...
    move = item_data.move
    next 0 if !move
    pbMessage("\\se[PC access]" + _INTL("启动了{1}！", item_data.name) + "\1")
    next 0 if !pbConfirmMessage(_INTL("要让教{1}给宝可梦吗？",
...
ItemHandlers::ConfirmUseInField.add(:ESCAPEROPE, proc { |item|
  ...
  next pbConfirmMessage(_INTL("想离开这里回到{1}吗？", mapname))
})
...
EventHandlers.add(:on_player_step_taken, :repel_counter,
  proc {
    ...
    if repels.none? { |item| $bag.has?(item) }
      pbMessage(_INTL("喷雾剂失去效果了！"))
      next
    end
    next if !pbConfirmMessage(_INTL("喷雾剂失去效果了！想再用一个吗？"))
    ret = nil
    pbFadeOutIn do
...
ItemHandlers::UseInField.add(:SACREDASH, proc { |item|
  ...
  pbFadeOutIn do
    ...
    screen.pbDisplay(_INTL("即便使用也无效果哦。")) if revived == 0
    screen.pbEndScene
  end
  next (revived > 0)
})
...
ItemHandlers::UseInField.add(:ITEMFINDER, proc { |item|
  ...
  if !event
    pbMessage(_INTL("……\wt[10]……\wt[10]……\wt[10]……\wt[10]什么都没有发生。"))
    next true
  end
  ...
  if offsetX == 0 && offsetY == 0   # Standing on the item, spin around
    ...
    pbWait(0.3)
    pbMessage(_INTL("{1}指向的是……脚下！", GameData::Item.get(item).name))
    ...
    pbWait(0.3)
    pbMessage(_INTL("啊？{1}有反应了！", GameData::Item.get(item).name) + "\1")
...
ItemHandlers::UseOnPokemon.add(:AWAKENING, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}醒过来了！", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ANTIDOTE, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}的毒消失得干干净净！", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:BURNHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}的灼伤治愈了！", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ICEHEAL, proc { |item, qty, pkmn, scene|
  ...
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}的冻冰被融化了！", pkmn.name))
  next true
})
...
ItemHandlers::UseOnPokemon.add(:ETHER, proc { |item, qty, pkmn, scene|
  move = scene.pbChooseMove(pkmn, _INTL("回复哪个招式？"))
...
ItemHandlers::UseOnPokemon.add(:PPUP, proc { |item, qty, pkmn, scene|
  move = scene.pbChooseMove(pkmn, _INTL("增加哪个招式的PP？"))
  next false if move < 0
...
ItemHandlers::UseOnPokemon.add(:POMEGBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :HP, qty, [
      _INTL("{1}十分喜欢你！它的基础HP降低了！", pkmn.name),
      _INTL("{1}更加喜欢你了。它的基础HP不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。但是它的基础HP降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:KELPSYBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :ATTACK, qty, [
      _INTL("{1}十分喜欢你！它的攻击降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。它的攻击不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。但是它的攻击降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:QUALOTBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :DEFENSE, qty, [
      _INTL("{1}十分喜欢你！它的防御降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。它的防御不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。但是它的防御降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:HONDEWBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPECIAL_ATTACK, qty, [
      _INTL("{1}十分喜欢你！它的特攻降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。它的特攻不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。但是它的特攻降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:GREPABERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPECIAL_DEFENSE, qty, [
      _INTL("{1}十分喜欢你！它的特防降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。它的特防不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。\n但是它的特防降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:TAMATOBERRY, proc { |item, qty, pkmn, scene|
  next pbRaiseHappinessAndLowerEV(
    pkmn, scene, :SPEED, qty, [
      _INTL("{1}十分喜欢你！它的速度降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。它的速度不能再降低了。", pkmn.name),
      _INTL("{1}更加喜欢你了。但是它的速度降低了。", pkmn.name)
    ]
  )
})
...
ItemHandlers::UseOnPokemon.add(:ABILITYCAPSULE, proc { |item, qty, pkmn, scene|
  if scene.pbConfirm(_INTL("你想要改变{1}的特性吗？", pkmn.name))
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}的特性改变了！现在它的特性是{2}！", pkmn.name, newabilname))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:ABILITYPATCH, proc { |item, qty, pkmn, scene|
  if scene.pbConfirm(_INTL("你想要改变{1}的特性吗？", pkmn.name))
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}的特性改变了！现在它的特性是{2}！", pkmn.name, new_ability_name))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:ROTOMCATALOG, proc { |item, qty, pkmn, scene|
  ...
  new_form = scene.pbShowCommands(_INTL("您想要哪种电器？"), choices, pkmn.form)
...
ItemHandlers::UseOnPokemon.add(:ZYGARDECUBE, proc { |item, qty, pkmn, scene|
  ...
  case scene.pbShowCommands(_INTL("对{1}做什么？", pkmn.name),
                            [_INTL("改变形态"), _INTL("改变特性"), _INTL("取消")])
  when 0   # Change form
    newForm = (pkmn.form == 0) ? 1 : 0
    pkmn.setForm(newForm) do
      scene.pbRefresh
  ...
  when 1   # Change ability
    ...
    pkmn.ability = nil
    scene.pbRefresh
    scene.pbDisplay(_INTL("{1}的特性改变了！现在它的特性是{2}！", pkmn.name, pkmn.ability.name))
    next true
  end
  next false
})
...
ItemHandlers::UseOnPokemon.add(:NLUNARIZER, proc { |item, qty, pkmn, scene|
  ...
  if pkmn == other_pkmn
    ...
    scene.pbDisplay(_INTL("不能与濒死的宝可梦融合。"))
    next false
