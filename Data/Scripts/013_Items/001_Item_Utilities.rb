...
def pbChangeLevel(pkmn, new_level, scene)
  ...
  if pkmn.level == new_level
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}的等级保持不变。", pkmn.name))
    else
      pbMessage(_INTL("{1}的等级保持不变。", pkmn.name))
    end
    return
  end
  ...
  if old_level > new_level
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}下降到了等级{2}！", pkmn.name, pkmn.level))
    else
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}上升到了等级{2}！", pkmn.name, pkmn.level))
    else
...
def pbChangeExp(pkmn, new_exp, scene)
  ...
  if pkmn.exp == new_exp
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}的经验值保持不变。", pkmn.name))
    else
      pbMessage(_INTL("{1}的经验值保持不变。", pkmn.name))
    end
    return
  end
  old_level           = pkmn.level
  ...
  if pkmn.exp > new_exp   # Loses Exp
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}失去了{2}经验值！", pkmn.name, difference))
    else
    ...
    if scene.is_a?(PokemonPartyScreen)
      scene.pbDisplay(_INTL("{1}获得了{2}经验值！", pkmn.name, difference))
    else
...
def pbHPItem(pkmn, restoreHP, scene)
  ...
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}回复了{2}点HP。", pkmn.name, hpGain))
  return true
end
...
def pbBattleHPItem(pkmn, battler, restoreHP, scene)
  if battler
    if battler.pbRecoverHP(restoreHP) > 0
      scene.pbDisplay(_INTL("{1}的体力回复了！", battler.pbThis))
    end
  elsif pbItemRestoreHP(pkmn, restoreHP) > 0
    scene.pbDisplay(_INTL("{1}的体力回复了！", pkmn.name))
  end
  return true
end
...
def pbUseEVRaisingItem(stat, amt_per_use, qty, pkmn, happiness_type, scene, no_ev_cap = false)
  ...
  pbSEPlay("Use item in party")
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}的{2}提高了！", pkmn.name, GameData::Stat.get(stat).name))
  return true
end
...
def pbNatureChangingMint(new_nature, item, pkmn, scene)
  ...
  if !scene.pbConfirm(_INTL("它可能会影响{1}的能力值，确定要使用它吗？", pkmn.name))
    return false
  end
  pkmn.nature_for_stats = new_nature
  pkmn.calc_stats
  scene.pbRefresh
  scene.pbDisplay(_INTL("由于{2}的影响，{1}的能力值可能发生了变化！",
...
def pbLearnMove(pkmn, move, ignore_if_known = false, by_machine = false, &block)
  ...
  if pkmn.egg? && !$DEBUG
    pbMessage(_INTL("蛋不能学习招式。"), &block)
    return false
  elsif pkmn.shadowPokemon?
    pbMessage(_INTL("黑暗宝可梦不能学习招式。"), &block)
    return false
  end
  pkmn_name = pkmn.name
  move_name = GameData::Move.get(move).name
  if pkmn.hasMove?(move)
    pbMessage(_INTL("{1}已经学会了{2}。", pkmn_name, move_name), &block) if !ignore_if_known
    return false
  elsif pkmn.numMoves < Pokemon::MAX_MOVES
    pkmn.learn_move(move)
    pbMessage("\\se[]" + _INTL("{1}学会了{2}", pkmn_name, move_name) + "\\se[Pkmn move learnt]", &block)
    return true
  end
  pbMessage(_INTL("{1}想要学习{2}，但是它已经学会了{3}个招式。",
                  pkmn_name, move_name, pkmn.numMoves.to_word) + "\1", &block)
  if pbConfirmMessage(_INTL("为了学习{2}，{1}要忘记招式吗？", pkmn_name, move_name), &block)
    loop do
      ...
      if move_index >= 0
        ...
        pbMessage(_INTL("１，２……空！") + "\\se[Battle ball drop]\1", &block)
        pbMessage(_INTL("{1}忘记了{2}。并且…" + "\1", pkmn_name, old_move_name), &block)
        pbMessage("\\se[]" + _INTL("{1}学会了{2}！", pkmn_name, move_name) + "\\se[Pkmn move learnt]", &block)
  ...
  if useType == 1   # Item is usable on a Pokémon
    ...
    if itm.is_evolution_stone?
      ...
      $player.party.each do |pkmn|
        ...
        annot.push((elig) ? _INTL("可选择") : _INTL("不可选择"))
      end
    end
    pbFadeOutIn do
      ...
      loop do
        ...
        if max_at_once > 1
          qty = screen.scene.pbChooseNumber(
            _INTL("你要使用多少{1}？", GameData::Item.get(item).portion_name_plural), max_at_once
          )
        ...
        pbMessage(_INTL("最后的{1}被使用了。", itm.portion_name)) { screen.pbUpdate }
        break
      end
      screen.pbEndScene
      bagscene&.pbRefresh
...
def pbUseItemOnPokemon(item, pkmn, scene)
  ...
  if itm.is_machine?
    ...
    if pkmn.shadowPokemon?
      pbMessage(_INTL("黑暗宝可梦不能学习招式。")) { scene.pbUpdate }
    elsif !pkmn.compatible_with_move?(machine)
      pbMessage(_INTL("{1}不能学习{2}。", pkmn.name, movename)) { scene.pbUpdate }
    else
      pbMessage("\\se[PC access]" + _INTL("启动了{1}！", itm.portion_name) + "\1") { scene.pbUpdate }
      if pbConfirmMessage(_INTL("要让{2}学习{1}吗？", movename, pkmn.name)) { scene.pbUpdate }
  ...
  if max_at_once > 1
    qty = scene.scene.pbChooseNumber(
      _INTL("你要使用多少{1}？", itm.portion_name_plural), max_at_once
    )
    scene.scene.pbSetHelpText("") if scene.is_a?(PokemonPartyScreen)
  end
  ...
  if ret && itm.consumed_after_use?
    $bag.remove(item, qty)
    if !$bag.has?(item)
      pbMessage(_INTL("最后的{1}被使用了。", itm.portion_name)) { scene.pbUpdate }
    end
  end
  return ret
end
...
def pbGiveItemToPokemon(item, pkmn, scene, pkmnid = 0)
  ...
  if pkmn.egg?
    scene.pbDisplay(_INTL("蛋不能携带道具。"))
    return false
  elsif pkmn.mail
    scene.pbDisplay(_INTL("给予道具前，必须先拿回{1}身上的邮件。", pkmn.name))
    return false if !pbTakeItemFromPokemon(pkmn, scene)
  end
  if pkmn.hasItem?
    olditemname = pkmn.item.portion_name
    if newitemname.starts_with_vowel?
      scene.pbDisplay(_INTL("{1}已经携带{2}了。", pkmn.name, olditemname) + "\1")
    else
      scene.pbDisplay(_INTL("{1}已经携带{2}了。", pkmn.name, olditemname) + "\1")
    end
    if scene.pbConfirm(_INTL("想要交换这两个道具吗？"))
      ...
      if !$bag.add(pkmn.item)
        raise _INTL("无法将已删除的道具重新放入包中") if !$bag.add(item)
        scene.pbDisplay(_INTL("背包已经满了，无法拿回宝可梦身上的道具。"))
      elsif GameData::Item.get(item).is_mail?
        if pbWriteMail(item, pkmn, pkmnid, scene)
          pkmn.item = item
          scene.pbDisplay(_INTL("从{2}身上拿回{1}了，并给予了{3}。", olditemname, pkmn.name, newitemname))
          return true
        elsif !$bag.add(item)
          raise _INTL("无法将已删除的道具重新放入包中")
        end
      else
        pkmn.item = item
        scene.pbDisplay(_INTL("从{2}身上拿回{1}了，并给予了{3}。", olditemname, pkmn.name, newitemname))
...
def pbTakeItemFromPokemon(pkmn, scene)
  ret = false
  if !pkmn.hasItem?
    scene.pbDisplay(_INTL("{1}现在什么也没有。", pkmn.name))
    ...
    elsif scene.pbConfirm(_INTL("如果邮件被删除，那么它携带的信息将丢失。确定吗？"))
      $bag.add(pkmn.item)
      scene.pbDisplay(_INTL("从{2}那接收了{1}。", pkmn.item.portion_name, pkmn.name))
      pkmn.item = nil
      pkmn.mail = nil
      ret = true
    end
