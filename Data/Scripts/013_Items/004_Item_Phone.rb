...
class Phone
  ...
  def self.add(*args)
    ...
    if ret
      ...
      pbMessage(_INTL("\\me[Register phone]在宝可装置中登记了{1}！", contact.display_name) + "\\wtnp[60]")
    end
    return ret
  end
...
class Phone
  module Call
    ...
    def make_outgoing(*args)
      ...
      raise _INTL("找不到提供的电话联系人：{1}。", args.inspect) if !contact
...
def pbPhoneRegisterBattle(message, event, trainer_type, name, versions_count)
  ...
  message = _INTL("让我给你登记。") if !message
