...
def pbDisplayGoldWindow(msgwindow)
  ...
  goldwindow = Window_AdvancedTextPokemon.new(_INTL("金钱：\n<ar>{1}</ar>", moneyString))
...
def pbDisplayCoinsWindow(msgwindow, goldwindow)
  ...
  coinwindow = Window_AdvancedTextPokemon.new(_INTL("硬币：\n<ar>{1}</ar>", coinString))
...
def pbDisplayBattlePointsWindow(msgwindow)
  ...
  pointswindow = Window_AdvancedTextPokemon.new(_INTL("对战点数：\n<ar>{1}</ar>", pointsString))
