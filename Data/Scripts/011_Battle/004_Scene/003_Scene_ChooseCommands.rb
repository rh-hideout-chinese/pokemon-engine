class Battle::Scene
  ...
  def pbCommandMenu(idxBattler, firstAction)
    ...
    cmds = [
      _INTL("{1}\n该怎么办？", @battle.battlers[idxBattler].name),
      _INTL("战斗"),
      _INTL("背包"),
      _INTL("宝可梦"),
  ...
  def pbItemMenu(idxBattler, _firstAction)
    ...
    loop do
      ...
      commands[commands.length]          = _INTL("取消")
      command = itemScene.pbShowCommands(_INTL("{1}被选中了", itemName), commands)
      ...
      when 1, 2, 3   # Use on Pokémon/Pokémon's move/battler
        ...
        pkmnScreen.pbStartScene(_INTL("对哪个宝可梦使用？"), @battle.pbNumPositions(0, 0))
        idxParty = -1
        # Loop while in party screen
        loop do
          # Select a Pokémon
          pkmnScene.pbSetHelpText(_INTL("对哪个宝可梦使用？"))
          ...
          if useType == 2   # Use on Pokémon's move
            idxMove = pkmnScreen.pbChooseMove(pkmn, _INTL("回复哪个招式？"))
            next if idxMove < 0
          end
