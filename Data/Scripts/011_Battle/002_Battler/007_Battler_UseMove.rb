  ...
  def pbProcessTurn(choice, tryFlee = true)
    ...
    if choice[0] == :Shift
      ...
      if idxOther >= 0
        ...
        case @battle.pbSideSize(@index)
        when 2
          @battle.pbDisplay(_INTL("{1}移动到了另一边！", pbThis))
        when 3
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    if isSpecies?(:AEGISLASH) && self.ability == :STANCECHANGE
      if move.damagingMove?
        pbChangeForm(1, _INTL("{1}，刀剑形态，变形！", pbThis))
      elsif move.id == :KINGSSHIELD
        pbChangeForm(0, _INTL("{1}，盾牌形态，变形！", pbThis))
      end
    end
    ...
    if move.snatched
      ...
      @battle.pbDisplay(_INTL("{1}抢夺了{2}的招式！", user.pbThis, pbThis(true)))
    end
    ...
    if user.status == :FROZEN && move.thawsUser?
      ...
      @battle.pbDisplay(_INTL("{1}融化了冻冰！", user.pbThis))
    end
    # Powder
    ...
    if move.damagingMove?
      ...
      when :HarshSun
        if move.calcType == :WATER
          @battle.pbDisplay(_INTL("受日照的影响，水属性的攻击被蒸发了！"))
          user.lastMoveFailed = true
          pbCancelMoves
          pbEndTurn(choice)
          return
    ...
    if targets.length == 0 && move.pbTarget(user).num_targets > 0 && !move.worksWithNoTargets?
      ...
      targets.each do |b|
        ...
        @battle.pbDisplay(_INTL("{1}被{2}弹开了！", b.pbThis, move.name))
      ...
      if magicCoater >= 0 || magicBouncer >= 0
        ...
        if !mc.fainted?
          ...
          @battle.pbDisplay(_INTL("{1}被{2}弹开了！", mc.pbThis, move.name))
