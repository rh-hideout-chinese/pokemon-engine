class Battle::Battler
  ...
  def pbMoveImmunityStatRaisingAbility(user, move, moveType, immuneType, stat, increment, show_message)
    ...
    if show_message
      ...
      if pbCanRaiseStatStage?(stat, self)
        ...
        @battle.pbDisplay(_INTL("对于{1}，好像没有效果……", pbThis(true)))
      else
        @battle.pbDisplay(_INTL("因为{1}的{2}，{3}无效！", pbThis, abilityName, move.name))
      end
      @battle.pbHideAbilitySplash(self)
    end
    return true
    ...
    if show_message
      ...
      if canHeal? && pbRecoverHP(@totalhp / 4) > 0
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("因为{2}，{1}回复了体力！", pbThis, abilityName))
        end
      elsif Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("对于{1}，好像没有效果……", pbThis(true)))
      else
  ...
  def pbOnLosingAbility(oldAbil, suppressed = false)
    if oldAbil == :NEUTRALIZINGGAS && (suppressed || !@effects[PBEffects::GastroAcid])
      ...
      if !@effects[PBEffects::Transform]
        ...
        @battle.pbDisplay(_INTL("{1}的{2}解除了！", pbThis, GameData::Ability.get(oldAbil).name))
  ...
  def pbSymbiosis
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("{1}和{3}平分了{2}！",
                                b.pbThis, b.itemName, pbThis(true)))
      else
        @battle.pbDisplay(_INTL("因为{2}，{1}和{4}平分了{3}！",
  ...
  def pbConfusionBerry(item_to_use, forced, confuse_stat, confuse_msg)
    ...
    if amt > 0
      if forced
        ...
        @battle.pbDisplay(_INTL("{1}用{2}回复了体力！", pbThis, used_item_name))
      end
    end
