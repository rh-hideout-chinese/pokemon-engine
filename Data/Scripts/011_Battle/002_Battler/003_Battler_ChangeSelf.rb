class Battle::Battler
  ...
  def pbReduceHP(amt, anim = true, registerDamage = true, anyAnim = true)
    ...
    raise _INTL("HP大于总HP") if @hp > @totalhp
  ...
  def pbCheckFormOnMovesetChange
    ...
    if isSpecies?(:KELDEO)
      ...
      pbChangeForm(newForm, _INTL("{1}的样子发生了变化！", pbThis))
    end
  end
    ...
    if isSpecies?(:MINIOR) && self.ability == :SHIELDSDOWN
      if @hp > @totalhp / 2   # Turn into Meteor form
        ...
        if @form != newForm
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(newForm, _INTL("{1}启动！", abilityName))
        end
      elsif @form < 7   # Turn into Core form
        @battle.pbShowAbilitySplash(self, true)
        @battle.pbHideAbilitySplash(self)
        pbChangeForm(@form + 7, _INTL("{1}解除！", abilityName))
      end
    end
