class Battle::Battler
  ...
  def pbCanRaiseStatStage?(stat, user = nil, move = nil, showFailMsg = false, ignoreContrary = false)
    ...
    if statStageAtMax?(stat)
      if showFailMsg
        @battle.pbDisplay(_INTL("{1}的{2}已经无法再提高了！",
  ...
  def pbRaiseStatStage(stat, increment, user, showAnim = true, ignoreContrary = false)
    ...
    arrStatTexts = [
      _INTL("{1}的{2}提高了！", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}的{2}大幅提高了！", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}的{2}巨幅提高了！", pbThis, GameData::Stat.get(stat).name)
    ]
  ...
  def pbRaiseStatStageByCause(stat, increment, user, cause, showAnim = true, ignoreContrary = false)
    ...
    if user.index == @index
      arrStatTexts = [
        _INTL("{1}的{2}提高了{3}！", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}的{2}大幅提高了{3}！", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}的{2}巨幅提高了{3}！", pbThis, cause, GameData::Stat.get(stat).name)
      ]
    else
      arrStatTexts = [
        _INTL("{1}的{2}提高了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}的{2}大幅提高了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}的{2}巨幅提高了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name)
      ]
    end
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
    # Trigger abilities upon stat gain
  ...
  def pbCanLowerStatStage?(stat, user = nil, move = nil, showFailMsg = false,
    ...
    if statStageAtMin?(stat)
      if showFailMsg
        @battle.pbDisplay(_INTL("{1}的{2}已经无法再降低了！",
                                pbThis, GameData::Stat.get(stat).name))
      end
      return false
    end
  ...
  def pbLowerStatStage(stat, increment, user, showAnim = true, ignoreContrary = false,
    ...
    if !@battle.moldBreaker
      ...
      if hasActiveAbility?(:MIRRORARMOR) && !ignoreMirrorArmor &&
        ...
        if mirrorArmorSplash < 2
          ...
          if !Battle::Scene::USE_ABILITY_SPLASH
            @battle.pbDisplay(_INTL("{1}的{2}发动！", pbThis, abilityName))
          end
        end
        ret = false
    ...
    arrStatTexts = [
      _INTL("{1}的{2}降低了！", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}的{2}大幅降低了！", pbThis, GameData::Stat.get(stat).name),
      _INTL("{1}的{2}巨幅降低了！", pbThis, GameData::Stat.get(stat).name)
    ]
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
  ...
  def pbLowerStatStageByCause(stat, increment, user, cause, showAnim = true,
    ...
    if user.index == @index
      arrStatTexts = [
        _INTL("{1}的{2}降低了{3}！", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}的{2}大幅降低了{3}！", pbThis, cause, GameData::Stat.get(stat).name),
        _INTL("{1}的{2}巨幅降低了{3}！", pbThis, cause, GameData::Stat.get(stat).name)
      ]
    else
      arrStatTexts = [
        _INTL("{1}的{2}降低了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}的{2}大幅降低了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name),
        _INTL("{1}的{2}巨幅降低了{3}的{4}！", user.pbThis, cause, pbThis(true), GameData::Stat.get(stat).name)
      ]
    end
    @battle.pbDisplay(arrStatTexts[[increment - 1, 2].min])
    # Trigger abilities upon stat loss
  ...
  def pbLowerAttackStatStageIntimidate(user)
    ...
    if @effects[PBEffects::Substitute] > 0
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        @battle.pbDisplay(_INTL("替身保护{1}免于被{2}的{3}影响！",
    ...
    if Settings::MECHANICS_GENERATION >= 8 && hasActiveAbility?([:OBLIVIOUS, :OWNTEMPO, :INNERFOCUS, :SCRAPPY])
      @battle.pbShowAbilitySplash(self)
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("{1}的{2}不能降低！", pbThis, GameData::Stat.get(:ATTACK).name))
      else
        @battle.pbDisplay(_INTL("{1}的{2}防止了{3}降低！", pbThis, abilityName,
    ...
    if !hasActiveAbility?(:CONTRARY)
      if pbOwnSide.effects[PBEffects::Mist] > 0
        @battle.pbDisplay(_INTL("{1}正受到白雾的保护，没有被{2}的{3}影响！",
                                pbThis, user.pbThis(true), user.abilityName))
        return false
      end
      if abilityActive? &&
        ...
        @battle.pbDisplay(_INTL("{1}的{2}阻止了{3}的{4}生效！",
      ...
      allAllies.each do |b|
        ...
        if Battle::AbilityEffects.triggerStatLossImmunityFromAlly(b.ability, b, self, :ATTACK, @battle, false)
          @battle.pbDisplay(_INTL("{1}因为{4}的{5}，没有被{2}的{3}影响！",
