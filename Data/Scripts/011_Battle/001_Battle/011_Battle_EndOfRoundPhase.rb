class Battle
  ...
  def pbEOREndWeather(priority)
    ...
    if @field.weatherDuration == 0
      ...
      when :ShadowSky then pbDisplay(_INTL("天空不再黑暗！"))
      end
      @field.weather = :None
    ...
    case @field.weather
    when :Sun         then pbDisplay(_INTL("烈日炎炎！"))
    when :Rain        then pbDisplay(_INTL("雨仍然在下！"))
    when :Sandstorm   then pbDisplay(_INTL("沙暴肆虐！"))
    when :Hail        then pbDisplay(_INTL("冰雹漫天！"))
    when :HarshSun    then pbDisplay(_INTL("日照非常强！"))
    when :HeavyRain   then pbDisplay(_INTL("暴雨倾盆！"))
    when :StrongWinds then pbDisplay(_INTL("狂风呼啸！"))
    when :ShadowSky   then pbDisplay(_INTL("仍被黑暗的天空笼罩！"))
    end
  ...
  def pbEORWeatherDamage(battler)
    ...
    when :ShadowSky
      ...
      pbDisplay(_INTL("黑暗袭击了{1}！", battler.pbThis))
      amt = battler.totalhp / 16
    end
    return if amt < 0
  ...
  def pbEORUseFutureSight(position, position_index)
    ...
    pbDisplay(_INTL("{1}吸引了{2}的攻击！", @battlers[position_index].pbThis,
  ...
  def pbEORWishHealing
    @positions.each_with_index do |pos, idxPos|
      ...
      pbDisplay(_INTL("{1}的祈愿实现了！", wishMaker))
    end
  end
  ...
  def pbEORHealingEffects(priority)
    ...
    priority.each do |battler|
      ...
      pbDisplay(_INTL("水流环回复了{1}的体力！", battler.pbThis(true)))
    end
    # Ingrain
    priority.each do |battler|
    ...
    priority.each do |battler|
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 8) do |hp_lost|
        recipient.pbRecoverHPFromDrain(hp_lost, battler,
                                       _INTL("寄生植物夺取了{1}的体力！", battler.pbThis))
      ...
      if battler.hasActiveAbility?(:POISONHEAL)
        if battler.canHeal?
          ...
          if Scene::USE_ABILITY_SPLASH
            ...
            pbDisplay(_INTL("因为{2}，{1}回复了体力！", battler.pbThis, battler.abilityName))
          end
  ...
  def pbEORTrappingDamage(battler)
    ...
    if battler.effects[PBEffects::Trapping] == 0
      pbDisplay(_INTL("{1}摆脱了{2}！", battler.pbThis, move_name))
      return
    end
  ...
  def pbEOREndBattlerEffects(priority)
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Taunt) do |battler|
      pbDisplay(_INTL("{1}的挑衅消失了！", battler.pbThis))
    end
    # Encore
    priority.each do |battler|
    ...
    pbEORCountDownBattlerEffect(priority, PBEffects::Embargo) do |battler|
      pbDisplay(_INTL("{1}无法使用道具了！", battler.pbThis))
    ...
    priority.each do |battler|
      ...
      pbDisplay(_INTL("{1}的灭亡计时变成{2}了！", battler.pbThis, battler.effects[PBEffects::PerishSong]))
  ...
  def pbEOREndSideEffects(side, priority)
    ...
    pbEORCountDownSideEffect(side, PBEffects::Reflect,
                             _INTL("{1}的反射壁消失了！", @battlers[side].pbTeam))
    # Light Screen
    ...
    pbEORCountDownSideEffect(side, PBEffects::LuckyChant,
                             _INTL("{1}的幸运咒语解除了！", @battlers[side].pbTeam))
    # Pledge Rainbow
    pbEORCountDownSideEffect(side, PBEffects::Rainbow,
                             _INTL("彩虹从{1}的上空消失了！", @battlers[side].pbTeam(true)))
    # Pledge Sea of Fire
    ...
    pbEORCountDownSideEffect(side, PBEffects::Swamp,
                             _INTL("{1}周围的湿地消失不见了！", @battlers[side].pbTeam(true)))
    # Aurora Veil
    pbEORCountDownSideEffect(side, PBEffects::AuroraVeil,
                             _INTL("{1}的极光幕消失了！", @battlers[side].pbTeam))
  end
  ...
  def pbEOREndTerrain
    ...
    if @field.terrain != :None && @field.terrainDuration == 0
      case @field.terrain
      when :Electric
        pbDisplay(_INTL("脚下的电光消失不见了！"))
      when :Grassy
        pbDisplay(_INTL("脚下的青草消失不见了！"))
      when :Misty
        pbDisplay(_INTL("脚下的雾气消失不见了！"))
      when :Psychic
        pbDisplay(_INTL("脚下的奇妙感觉消失了！"))
      end
      @field.terrain = :None
  ...
  def pbEOREndBattlerSelfEffects(battler)
    ...
    if battler.inHyperMode?
      if pbRandom(100) < 10
        ...
        pbDisplay(_INTL("{1}清醒过来了！", battler.pbThis))
      else
        pbDisplay(_INTL("{1}进入了暴走状态！", battler.pbThis))
      end
    end
    # Uproar
    if battler.effects[PBEffects::Uproar] > 0
      ...
      if battler.effects[PBEffects::Uproar] == 0
        pbDisplay(_INTL("{1}冷静下来了！", battler.pbThis))
      else
  ...
  def pbEORShiftDistantBattlers
    ...
    if !singleBattle?
      ...
      swaps.each do |pair|
        ...
        when 3
          pbDisplay(_INTL("{1}移动到了中间！", @battlers[pair[1]].pbThis))
        end
      end
    end
  end
  ...
  def pbEndOfRoundPhase
    ...
    if Settings::AFFECTION_EFFECTS && @internalBattle
      priority.each do |battler|
        ...
        when :FROZEN
          pbDisplay(_INTL("为了不让你担心，{1}努力融化了冻冰！", battler.pbThis))
        end
      end
    end
    ...
    priority.each do |battler|
      ...
      pbDisplay(_INTL("因为处于暴走状态，{1}受到了伤害！", battler.pbThis(true)))
      battler.pbFaint if battler.fainted?
    end
