class Battle
      ...
      if numPartic > 0 || expShare.length > 0 || expAll
        ...
        if expAll
          ...
          eachInTeam(0, 0) do |pkmn, i|
            ...
            pbDisplayPaused(_INTL("你其他的宝可梦也获得了经验值！")) if showMessage
            showMessage = false
            pbGainEVsOne(i, b)
  ...
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    loop do   # For each level gained in turn...
      ...
      battler&.pbUpdate(false)
      @scene.pbRefreshOne(battler.index) if battler
      pbDisplayPaused(_INTL("{1}上升到了等级{2}！", pkmn.name, curLevel)) { pbSEPlay("Pkmn level up") }
  ...
  def pbLearnMove(idxParty, newMove)
    ...
    pbDisplayPaused(_INTL("{1}想要学习{2}，但是它已经学会了{3}个招式。",
                          pkmnName, moveName, pkmn.numMoves.to_word))
    if pbDisplayConfirm(_INTL("为了学习{2}，{1}要忘记招式吗？", pkmnName, moveName))
      loop do
        ...
        if forgetMove >= 0
          ...
          pbDisplayPaused(_INTL("１，２……空！")) { pbSEPlay("Battle ball drop") }
          pbDisplayPaused(_INTL("{1}忘记了{2}。并且…", pkmnName, oldMoveName))
          pbDisplay(_INTL("{1}学会了{2}！", pkmnName, moveName)) { pbSEPlay("Pkmn move learnt") }
          battler&.pbCheckFormOnMovesetChange
          break
