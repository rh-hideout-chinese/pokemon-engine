...
class Battle
  ...
  def pbThisEx(idxBattler, idxParty)
    ...
    return _INTL("我方的{1}", party[idxParty].name) if !pbOwnedByPlayer?(idxBattler)
  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    when :ShadowSky   then pbDisplay(_INTL("天空变得黑暗！"))
    end
  ...
  def pbStartWeatherAbility(new_weather, battler, ignore_primal = false)
    ...
    if !Scene::USE_ABILITY_SPLASH
      pbDisplay(_INTL("{1}的{2}发动！", battler.pbThis, battler.abilityName))
    end
