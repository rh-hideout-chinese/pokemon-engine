class Battle
  ...
  def pbCanSwitchIn?(idxBattler, idxParty, partyScene = nil)
    ...
    if party[idxParty].egg?
      partyScene&.pbDisplay(_INTL("无法让蛋出场战斗。"))
      return false
    end
    if !pbIsOwner?(idxBattler, idxParty)
      if partyScene
        owner = pbGetOwnerFromPartyIndex(idxBattler, idxParty)
        partyScene.pbDisplay(_INTL("你不能用你的宝可梦替换{1}的宝可梦！", owner.name))
      end
      return false
    end
    if party[idxParty].fainted?
      partyScene&.pbDisplay(_INTL("{1}已没有精力再战了！", party[idxParty].name))
      return false
    end
    if pbFindBattler(idxParty, idxBattler)
      partyScene&.pbDisplay(_INTL("已经在战斗了。", party[idxParty].name))
      return false
    end
    return true
  end
  ...
  def pbCanSwitchOut?(idxBattler, partyScene = nil)
    ...
    allOtherSideBattlers(idxBattler).each do |b|
      ...
      if Battle::AbilityEffects.triggerTrappingByTarget(b.ability, battler, b, self)
        partyScene&.pbDisplay(_INTL("因{1}的{2}而无法进行替换！", b.pbThis, b.abilityName))
        return false
      end
    end
    allOtherSideBattlers(idxBattler).each do |b|
      ...
      if Battle::ItemEffects.triggerTrappingByTarget(b.item, battler, b, self)
        partyScene&.pbDisplay(_INTL("因{1}的{2}而无法进行替换！", b.pbThis, b.itemName))
        return false
      end
    end
    return true
  ...
  def pbCanSwitch?(idxBattler, idxParty = -1, partyScene = nil)
    ...
    allSameSideBattlers(idxBattler).each do |b|
      ...
      partyScene&.pbDisplay(_INTL("{1}已经被选中了。",
    ...
    loop do
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          switch = false
          if pbDisplayConfirm(_INTL("使用下只宝可梦？"))
            switch = true
          else
  ...
  def pbMessagesOnBattlerEnteringBattle(battler)
    ...
    if battler.shadowPokemon?
      ...
      pbDisplay(_INTL("噢！一个黑暗宝可梦！")) if battler.opposes?
    end
  end
  ...
  def pbEntryHazards(battler)
    ...
    if battler_side.effects[PBEffects::StickyWeb] && !battler.fainted? && !battler.airborne? &&
      ...
      pbDisplay(_INTL("{1}被黏黏网粘住了！", battler.pbThis))
