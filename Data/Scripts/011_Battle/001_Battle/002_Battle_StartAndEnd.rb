class Battle
  ...
  def pbEnsureParticipants
    ...
    if trainerBattle? && (@sideSizes[0] > 2 || @sideSizes[1] > 2) &&
      ...
      raise _INTL("双方有多个训练家，不能进行超过2对2的对战。")
    end
    ...
    loop do
      ...
      2.times do |side|   # Each side in turn
        ...
        if requireds.length > sideCounts.length
          raise _INTL("错误：def pbGetOwnerIndexFromBattlerIndex 为对战类型{2}v{3}，训练家{4}v{5} 提供了无效的索引{1}！",
        ...
        sideCounts.each_with_index do |_count, i|
          if !requireds[i] || requireds[i] == 0
            case side
            when 0
              raise _INTL("玩家方的训练家{1}没有可以让宝可梦上场的位置（开始{2}对{3}的对战时）",
                          i + 1, @sideSizes[0], @sideSizes[1])
            when 1
              raise _INTL("对手方的训练家{1}没有可以让宝可梦上场的位置（开始{2}对{3}的对战时）",
                          i + 1, @sideSizes[0], @sideSizes[1])
            end
          end
          next if requireds[i] <= sideCounts[i]   # Trainer has enough Pokémon to fill their positions
          if requireds[i] == 1
            raise _INTL("玩家方训练家{1}没有可以上场的宝可梦", i + 1) if side == 0
            raise _INTL("对手方训练家{1}没有可以上场的宝可梦", i + 1) if side == 1
          end
      ...
      if newSize == 0
        raise _INTL("不能再减少任何一方的队伍大小，无法开始对战")
      end
      2.times do |side|
  ...
  def pbCreateBattler(idxBattler, pkmn, idxParty)
    if !@battlers[idxBattler].nil?
      raise _INTL("索引{1}已经存在", idxBattler)
    end
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 1
        pbDisplayPaused(_INTL("噢！野生的{1}出现了！", foeParty[0].name))
      when 2
        pbDisplayPaused(_INTL("噢！野生的{1}和{2}出现了！", foeParty[0].name,
                              foeParty[1].name))
      when 3
        pbDisplayPaused(_INTL("噢！野生的{1}、{2}和{3}出现了！", foeParty[0].name,
    ...
    [1, 0].each do |side|
      ...
      trainers.each_with_index do |t, i|
        ...
        case sent.length
        when 1
          msg += _INTL("{1}派出了{2}！", t.full_name, @battlers[sent[0]].name)
        when 2
          msg += _INTL("{1}派出了{2}和{3}！", t.full_name,
                       @battlers[sent[0]].name, @battlers[sent[1]].name)
        when 3
          msg += _INTL("{1}派出了{2}、{3}和{4}！", t.full_name,
      ...
      if side == 0
        ...
        case sent.length
        when 1
          msg += _INTL("上吧！{1}！", @battlers[sent[0]].name)
        when 2
          msg += _INTL("上吧！{1}和{2}！", @battlers[sent[0]].name, @battlers[sent[1]].name)
        when 3
          msg += _INTL("上吧！{1}、{2}和{3}！", @battlers[sent[0]].name,
  ...
  def pbStartBattleCore
    ...
    when :Hail        then pbDisplay(_INTL("现在正在下冰雹！"))
  ...
  def pbGainMoney
    ...
    if trainerBattle?
      ...
      if moneyGained > 0
        ...
        pbDisplayPaused(_INTL("作为奖金，你获得了${1}！", moneyGained.to_s_formatted))
      end
    end
  ...
  def pbEndOfBattle
    ...
    when 1
      ...
      if trainerBattle?
        ...
        case @opponent.length
        when 1
          pbDisplayPaused(_INTL("你战胜了{1}！", @opponent[0].full_name))
        when 2
          pbDisplayPaused(_INTL("你战胜了{1}和{2}！", @opponent[0].full_name,
                                @opponent[1].full_name))
        when 3
          pbDisplayPaused(_INTL("你战胜了{1}、{2}和{3}！", @opponent[0].full_name,
    ...
    when 2, 5
      ...
      if @internalBattle
        ...
        if trainerBattle?
          case @opponent.length
          when 1
            pbDisplayPaused(_INTL("你被{1}打败了！", @opponent[0].full_name))
          when 2
            pbDisplayPaused(_INTL("你被{1}和{2}打败了！",
                                  @opponent[0].full_name, @opponent[1].full_name))
          when 3
            pbDisplayPaused(_INTL("你被{1}、{2}和{3}打败了！",
                                  @opponent[0].full_name, @opponent[1].full_name, @opponent[2].full_name))
          end
        end
        # Lose money from losing a battle
        pbLoseMoney
        pbDisplayPaused(_INTL("你的眼前变得一片漆黑！")) if !@canLose
