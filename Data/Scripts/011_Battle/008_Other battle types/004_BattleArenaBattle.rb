...
#    PBDebug.log("[Mind: #{@mind.inspect}, Skill: #{@skill.inspect}]")
    ...
    if points[0] == points[1]
      pbDisplay(_INTL("在裁判的判定中，{1}追平了{2}！",
      ...
      pbDisplay(_INTL("在裁判的判定中，{1}击败了{2}！",
                      @battlers[0].name, @battlers[1].name))
      @battlers[1].hp = 0
      @battlers[1].pbFaint(false)
    else
      pbDisplay(_INTL("在裁判的判定中，{1}输给了{2}！",
                      @battlers[0].name, @battlers[1].name))
      @battlers[0].hp = 0
      @battlers[0].pbFaint(false)
    end
  ...
  def pbBattleArenaJudgment(battler1, battler2, ratings1, ratings2)
    ...
    begin
      ...
      pbMessageDisplay(msgwindow,
                       _INTL("裁判：可以了！我们将会决定获胜者！") + "\\wtnp[10]") do
        pbBattleArenaUpdate
        dimmingvp.update
      end
      dimmingvp.z = 99999
