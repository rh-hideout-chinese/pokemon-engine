  ...
  def pbStartBattle
    begin
      ...
      loop do
        ...
        when 1   # Bait
          pbDisplayBrief(_INTL("{1}朝{2}扔了一些诱饵！", self.pbPlayer.name, pkmn.name))
          @scene.pbThrowBait
          catchFactor  /= 2 if pbRandom(100) < 90   # Harder to catch
          escapeFactor /= 2                       # Less likely to escape
        when 2   # Rock
          pbDisplayBrief(_INTL("{1}朝{2}扔了一块石头！", self.pbPlayer.name, pkmn.name))
          @scene.pbThrowRock
        ...
        if @decision == 0
          if @ballCount <= 0
            ...
            pbDisplay(_INTL("广播员：你已经没有狩猎球了！结束！"))
            @decision = 2
          elsif pbRandom(100) < 5 * escapeFactor
            pbSEPlay("Battle flee")
            pbDisplay(_INTL("{1}逃跑了！", pkmn.name))
            @decision = 3
          elsif cmd == 1   # Bait
            pbDisplay(_INTL("{1}正在吃诱饵！", pkmn.name))
          elsif cmd == 2   # Rock
            pbDisplay(_INTL("{1}非常生气！", pkmn.name))
          else
            pbDisplay(_INTL("{1}正在观察你！", pkmn.name))
          end
          # Weather continues
          weather_data = GameData::BattleWeather.try_get(@weather)
          @scene.pbCommonAnimation(weather_data.animation) if weather_data
