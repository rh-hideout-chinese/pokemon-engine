...
class Battle::Move::SleepTargetChangeUserMeloettaForm < Battle::Move::SleepTarget
  def pbEndOfMoveUsageEffect(user, targets, numHits, switchedBattlers)
    ...
    user.pbChangeForm(newForm, _INTL("{1}的样子发生了变化！", user.pbThis))
  end
end
...
class Battle::Move::GiveUserStatusToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    when :PARALYSIS
      target.pbParalyze(user)
      msg = _INTL("{1}的麻痹被解除了！", user.pbThis)
    when :FROZEN
      target.pbFreeze
      msg = _INTL("{1}的冻冰被融化了！", user.pbThis)
    end
    if msg != ""
...
class Battle::Move::CureUserBurnPoisonParalysis < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    case old_status
    when :BURN
      @battle.pbDisplay(_INTL("{1}治愈了灼伤！", user.pbThis))
    when :POISON
      @battle.pbDisplay(_INTL("{1}治愈了中毒！", user.pbThis))
    when :PARALYSIS
      @battle.pbDisplay(_INTL("{1}治愈了麻痹！", user.pbThis))
    end
  end
end
...
class Battle::Move::SetUserTypesBasedOnEnvironment < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("{1}的属性变成了{2}！", user.pbThis, typeName))
  end
end
...
class Battle::Move::SetUserTypesToTargetTypes < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("为匹配{2}的属性，{1}的属性已更改！",
...
class Battle::Move::AddGhostTypeToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}变成了{2}属性！", target.pbThis, typeName))
  end
end
...
class Battle::Move::UserTargetSwapAbilities < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if Battle::Scene::USE_ABILITY_SPLASH
      ...
      @battle.pbDisplay(_INTL("{1}将{2}与对手的{3}互换了！",
...
class Battle::Move::NegateTargetAbility < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}的特性变得无效了！", target.pbThis))
...
class Battle::Move::StartTargetAirborneAndAlwaysHitByMoves < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}浮在了空中！", target.pbThis))
  end
end
