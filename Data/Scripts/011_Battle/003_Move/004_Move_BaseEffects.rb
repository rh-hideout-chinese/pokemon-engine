...
class Battle::Move::MultiStatUpMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if failed
      @battle.pbDisplay(_INTL("{1}的能力已经无法再提高了！", user.pbThis))
      return true
    end
    return false
  end
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if failed
      ...
      if target.hasActiveAbility?(:CONTRARY) && !@battle.moldBreaker
        ...
        @battle.pbDisplay(_INTL("{1}的能力已经无法再提高了！", user.pbThis)) if !canLower && show_message
      else
        (@statDown.length / 2).times do |i|
        ...
        @battle.pbDisplay(_INTL("{1}的能力已经无法再降低了！", user.pbThis)) if !canLower && show_message
      end
      if canLower
...
class Battle::Move::TwoTurnMove < Battle::Move
  ...
  def pbQuickChargingMove(user, targets)
    ...
    if @powerHerb
      ...
      @battle.pbDisplay(_INTL("{1}用了强力香草后，\n充满了力量！", user.pbThis))
      user.pbConsumeItem
    end
  end
...
class Battle::Move::HealingMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.hp == user.totalhp
      @battle.pbDisplay(_INTL("{1}的体力是全满的！", user.pbThis))
      return true
    end
    return false
  end
...
class Battle::Move::ProtectMove < Battle::Move
  ...
  def pbProtectMessage(user)
    if @sidedEffect
      @battle.pbDisplay(_INTL("{1}守护了{2}！", @name, user.pbTeam(true)))
    else
      @battle.pbDisplay(_INTL("{1}在攻击中守护住了自己！", user.pbThis))
    end
  end
end
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    when :Rainbow   # Fire + Water
      if user.pbOwnSide.effects[PBEffects::Rainbow] == 0
        ...
        msg = _INTL("彩虹出现在了{1}上空！", user.pbTeam(true))
