class Battle::Move
  ...
  def pbMoveFailedLastInRound?(user, showMessage = true)
    ...
    if !unmoved
      @battle.pbDisplay(_INTL("但是，招式失败了！")) if showMessage
      return true
    end
    return false
  end
    ...
    target.allAllies.each do |b|
      ...
      if showMessage
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("因为{2}，对{1}没有效果！",
  ...
  def pbHitEffectivenessMessages(user, target, numTargets = 1)
    ...
    if target.damageState.critical
      ...
      if target.damageState.affection_critical
        if numTargets > 1
          @battle.pbDisplay(_INTL("为了得到夸奖，{1}在瞄准后击中了{2}的要害！",
                                  user.pbThis, target.pbThis(true)))
        else
          @battle.pbDisplay(_INTL("为了得到夸奖，{1}在瞄准后击中了要害！", user.pbThis))
        end
      elsif numTargets > 1
  ...
  def pbEndureKOMessage(target)
    if target.damageState.disguise
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        @battle.pbDisplay(_INTL("{1}的画皮变成了替身！", target.pbThis))
      end
      @battle.pbHideAbilitySplash(target)
      target.pbChangeForm(1, _INTL("{1}的画皮脱落了！", target.pbThis))
      ...
      target.pbChangeForm(1, _INTL("{1}的样子发生了变化！", target.pbThis))
      @battle.pbHideAbilitySplash(target)
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        @battle.pbDisplay(_INTL("{1}因结实撑住了！", target.pbThis))
      end
      @battle.pbHideAbilitySplash(target)
    elsif target.damageState.focusSash
      @battle.pbCommonAnimation("UseItem", target)
      @battle.pbDisplay(_INTL("{1}因气势披带撑住了！", target.pbThis))
      target.pbConsumeItem
    elsif target.damageState.focusBand
      @battle.pbCommonAnimation("UseItem", target)
      @battle.pbDisplay(_INTL("{1}因气势头带撑住了！", target.pbThis))
