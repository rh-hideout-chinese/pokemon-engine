...
class Battle::Move::SwitchOutTargetStatusMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if target.effects[PBEffects::Ingrain]
      @battle.pbDisplay(_INTL("{1}用根固定住了！", target.pbThis)) if show_message
      return true
    end
...
class Battle::Move::BindTarget < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    when :CLAMP
      msg = _INTL("{1}被{2}的贝壳夹住了！", user.pbThis, target.pbThis(true))
    when :FIRESPIN
      msg = _INTL("{1}被困在了火焰旋涡之中！", target.pbThis)
    when :INFESTATION
      msg = _INTL("{1}受到了{2}的死缠烂打！", target.pbThis, user.pbThis(true))
    when :MAGMASTORM
      msg = _INTL("{1}被困在了熔岩风暴之中！", target.pbThis)
    when :SANDTOMB
      msg = _INTL("{1}被困在了流沙地狱之中！", target.pbThis)
    when :WHIRLPOOL
      msg = _INTL("{1}被困在了旋涡之中！", target.pbThis)
    when :WRAP
...
class Battle::Move::TrapTargetInBattleLowerTargetDefSpDef1EachTurn < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("因为{2}，{1}已经无法逃走了！", target.pbThis, @name))
  end
end
...
class Battle::Move::UsedAfterUserTakesPhysicalDamage < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if !user.tookPhysicalHit
      @battle.pbDisplay(_INTL("{1}的陷阱甲壳没有被触发！", user.pbThis))
      return true
    end
    return false
  end
...
class Battle::Move::StartSlowerBattlersActFirst < Battle::Move
  def pbEffectGeneral(user)
    if @battle.field.effects[PBEffects::TrickRoom] > 0
      ...
      @battle.pbDisplay(_INTL("{1}复原了时空！", user.pbThis))
    else
...
class Battle::Move::LowerPPOfTargetLastMoveBy3 < Battle::Move
  def pbAdditionalEffect(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}的{2}减少了{3}！",
...
class Battle::Move::DisableTargetLastMoveUsed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("封住了{1}的{2}！", target.pbThis,
    ...
    if Settings::MECHANICS_GENERATION >= 6 && target.hasActiveAbility?(:OBLIVIOUS) &&
      ...
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("但是因为{1}的{2}失败了！",
...
class Battle::Move::DisableTargetSoundMoves < Battle::Move
  def pbAdditionalEffect(user, target)
    ...
    if target.effects[PBEffects::ThroatChop] == 0
      @battle.pbDisplay(_INTL("因{1}的效果，{2}无法使出某些招式！",
