...
class Battle::Move::UserTakesTargetItem < Battle::Move
  def pbEffectAfterAllHits(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}从{2}那里夺取了{3}！", user.pbThis, target.pbThis(true), itemName))
...
class Battle::Move::TargetTakesUserItem < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}从{3}那里获得了{2}！", target.pbThis, itemName, user.pbThis(true)))
...
class Battle::Move::UserTargetSwapItems < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}获得了{2}！", user.pbThis, oldTargetItemName)) if oldTargetItem
    @battle.pbDisplay(_INTL("{1}获得了{2}！", target.pbThis, oldUserItemName)) if oldUserItem
    user.pbHeldItemTriggerCheck
    target.pbHeldItemTriggerCheck
  end
end
...
class Battle::Move::RestoreUserConsumedItem < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    if itemName.starts_with_vowel?
      @battle.pbDisplay(_INTL("{1}找到{2}了！", user.pbThis, itemName))
    else
      @battle.pbDisplay(_INTL("{1}找到{2}了！", user.pbThis, itemName))
    end
    user.pbHeldItemTriggerCheck
...
class Battle::Move::DestroyTargetBerryOrGem < Battle::Move
  def pbEffectWhenDealingDamage(user, target)
    ...
    target.pbRemoveItem
    @battle.pbDisplay(_INTL("{1}的{2}被烧没了！", target.pbThis, item_name))
  end
end
...
class Battle::Move::CorrodeTargetItem < Battle::Move
    ...
    if target.hasActiveAbility?(:STICKYHOLD) && !@battle.moldBreaker
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("因为{1}的{2}，完全没有效果！",
                                  target.pbThis(true), target.abilityName))
        end
        @battle.pbHideAbilitySplash(target)
      end
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}把{2}的{3}融化了！",
...
class Battle::Move::UserConsumeBerryRaiseDefense2 < Battle::Move::StatUpMove
  ...
  def pbCanChooseMove?(user, commandPhase, showMessages)
    ...
    if !item || !item.is_berry? || !user.itemActive?
      if showMessages
        msg = _INTL("因为{1}没有树果，不能使用招式！", user.pbThis)
