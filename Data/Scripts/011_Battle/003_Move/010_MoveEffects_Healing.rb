...
class Battle::Move::HealTargetHalfOfTotalHP < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.hp == target.totalhp
      @battle.pbDisplay(_INTL("{1}的体力是全满的！", target.pbThis)) if show_message
      return true
...
class Battle::Move::StartHealUserEachTurn < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("{1}通过水环回复体力！", user.pbThis))
  end
end
...
class Battle::Move::StartLeechSeedTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.effects[PBEffects::LeechSeed] >= 0
      @battle.pbDisplay(_INTL("{1}避过了攻击！", target.pbThis)) if show_message
      return true
    end
  ...
  def pbMissMessage(user, target)
    @battle.pbDisplay(_INTL("{1}避过了攻击！", target.pbThis))
    return true
  end
...
class Battle::Move::StartPerishCountsForAllBattlers < Battle::Move
  ...
  def pbShowAnimation(id, user, targets, hitNum = 0, showAnimation = true)
    super
    @battle.pbDisplay(_INTL("听到这首歌的宝可梦3回合后会陷入昏厥！"))
  end
end
