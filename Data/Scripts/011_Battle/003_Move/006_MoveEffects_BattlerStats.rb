...
class Battle::Move::LowerUserDefSpDef1RaiseUserAtkSpAtkSpd2 < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if failed
      @battle.pbDisplay(_INTL("{1}的能力不能更改！", user.pbThis))
      return true
    end
    return false
  end
...
class Battle::Move::RaiseUserMainStats1TrapUserInBattle < Battle::Move::RaiseUserMainStats1
  ...
  def pbEffectGeneral(user)
    ...
    if !user.trappedInBattle?
      ...
      @battle.pbDisplay(_INTL("因为{2}，{1}已经无法逃走了！", user.pbThis, @name))
    end
  end
end
...
class Battle::Move::RaiseTargetRandomStat2 < Battle::Move
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if @statArray.length == 0
      @battle.pbDisplay(_INTL("{1}的{2}已经无法再提高了！", target.pbThis)) if show_message
      return true
    end
    return false
  end
    ...
    if target.hasActiveAbility?(:OBLIVIOUS) && !@battle.moldBreaker
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("{2}防止了{1}着迷！", target.pbThis, target.abilityName))
        end
...
class Battle::Move::LowerTargetEvasion1RemoveSideEffects < Battle::Move::TargetStatDownMove
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    if target.pbOwnSide.effects[PBEffects::Mist] > 0
      ...
      @battle.pbDisplay(_INTL("{1}的白雾消失了！", target.pbTeam))
    end
...
class Battle::Move::RaiseAlliesAtkDef1 < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    @battle.pbDisplay(_INTL("{1}的能力不能进一步提高了！", target.pbThis)) if show_message
    return true
  end
...
class Battle::Move::UserCopyTargetStatStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}复制了{2}的能力变化！", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::InvertTargetStatStages < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1}的能力颠倒过来了！", target.pbThis))
  end
end
...
class Battle::Move::ResetTargetStatStages < Battle::Move
  def pbEffectAgainstTarget(user, target)
    if target.damageState.calcDamage > 0 && !target.damageState.substitute &&
      ...
      @battle.pbDisplay(_INTL("{1}的能力变化消失了！", target.pbThis))
    end
  end
end
