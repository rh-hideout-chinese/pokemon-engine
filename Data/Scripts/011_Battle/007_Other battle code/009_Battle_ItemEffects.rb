...
Battle::ItemEffects::HPHeal.add(:AGUAVBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_DEFENSE,
       _INTL("对于{1}，{2}太苦了！", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:FIGYBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :ATTACK,
       _INTL("对于{1}，{2}太辣了！", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:IAPAPABERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :DEFENSE,
       _INTL("对于{1}，{2}太酸了！", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:LANSATBERRY,
  proc { |item, battler, battle, forced|
    ...
    itemName = GameData::Item.get(item).name
    if forced
      battle.pbDisplay(_INTL("{2}让{1}变得干劲十足了！", battler.pbThis, itemName))
    else
      battle.pbDisplay(_INTL("{1}使用了{2}，变得干劲十足了！", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:MAGOBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPEED,
       _INTL("对于{1}，{2}太甜了！", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:MICLEBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("{1}下一个使出的招式的命中率提高了！", battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1}使用了{2}，下一个使出的招式的命中率提高了！",
...
Battle::ItemEffects::HPHeal.add(:WIKIBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_ATTACK,
       _INTL("对于{1}，{2}太涩了！", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::OnStatLoss.add(:EJECTPACK,
  proc { |item, battler, move_user, battle|
    ...
    battle.pbDisplay(_INTL("{1}因{2}回去了！", battler.pbThis, battler.itemName))
...
Battle::ItemEffects::StatusCure.add(:ASPEARBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbDisplay(_INTL("因为{2}，{1}治愈了冰冻状态！", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:CHERIBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("因为{2}，{1}治愈了麻痹！", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:CHESTOBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("因为{2}，{1}醒过来了！", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:LUMBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      if oldConfusion
        battle.pbDisplay(_INTL("因为{2}，{1}的混乱解除了！", battler.pbThis, itemName))
      end
    end
    next true
  }
  ...
  proc { |item, battler, battle, forced|
    ...
    if battler.effects[PBEffects::Attract] >= 0
      if forced
        ...
        battle.pbDisplay(_INTL("{1}使用{2}治愈了着迷状态！",
           battler.pbThis, itemName))
      end
      battler.pbCureAttract
    end
    battle.pbDisplay(_INTL("{1}的挑衅消失了！", battler.pbThis)) if battler.effects[PBEffects::Taunt] > 0
    battler.effects[PBEffects::Taunt] = 0
...
Battle::ItemEffects::StatusCure.add(:PECHABERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("因为{2}，{1}治愈了中毒！", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:PERSIMBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("因为{2}，{1}的混乱解除了！", battler.pbThis,
         itemName))
    end
    next true
  }
...
Battle::ItemEffects::StatusCure.add(:RAWSTBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("因为{2}，{1}治愈了灼伤！", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::PriorityBracketUse.add(:CUSTAPBERRY,
  proc { |item, battler, battle|
    ...
    battle.pbDisplay(_INTL("{2}让{1}行动变快了！", battler.pbThis, battler.itemName))
...
Battle::ItemEffects::OnMissingTarget.add(:BLUNDERPOLICY,
  proc { |item, user, target, move, hit_num, battle|
    ...
    battle.pbDisplay(_INTL("{1}使用了……", user.itemName))
...
Battle::ItemEffects::OnBeingHit.add(:AIRBALLOON,
  proc { |item, user, target, move, battle|
    battle.pbDisplay(_INTL("{1}的{2}破了！", target.pbThis, target.itemName))
...
Battle::ItemEffects::OnBeingHit.add(:JABOCABERRY,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(_INTL("{1}消耗了{2}，{3}受到了伤害！", target.pbThis,
...
Battle::ItemEffects::OnBeingHit.add(:WEAKNESSPOLICY,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(_INTL("{1}使用了……", target.itemName))
    target.pbHeldItemTriggered(item)
  }
)
  ...
  proc { |item, battler, user, move, switched_battlers, battle|
    ...
    if user.hasActiveAbility?(:SUCTIONCUPS) && !battle.moldBreaker
      ...
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}用固定了！", user.pbThis))
      else
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:SHELLBELL,
  proc { |item, user, targets, move, numHits, battle|
    ...
    battle.pbDisplay(_INTL("{1}用{2}回复了少许HP。",
...
Battle::ItemEffects::OnEndOfUsingMove.add(:LEPPABERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      battle.pbDisplay(_INTL("{1}回复了{2}的ＰＰ！", battler.pbThis, moveName))
    else
      battle.pbDisplay(_INTL("{1}的{2}回复了{3}的ＰＰ！", battler.pbThis, itemName, moveName))
    end
    next true
  }
)
...
Battle::ItemEffects::OnEndOfUsingMoveStatRestore.add(:WHITEHERB,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("UseItem", battler) if !forced
    if forced
      battle.pbDisplay(_INTL("{1}的状态复原了！", battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1}使用{2}复原了状态！",
         battler.pbThis, itemName))
    end
    next true
  }
