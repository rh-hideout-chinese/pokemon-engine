...
class TriadScene
  ...
  def pbChooseTriadCard(cardStorage)
    ...
    @sprites["helpwindow"].text = _INTL("选择{1}张卡用于决斗。", @battle.maxCards)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) || Input.trigger?(Input::BACK)
        ...
        if chosenCards.length == @battle.maxCards
          @sprites["helpwindow"].text = _INTL("{1}卡已被选择。", @battle.maxCards)
  ...
  def pbViewOpponentCards(numCards)
    @sprites["helpwindow"].text = _INTL("查看对手的卡。")
    choice     = 0
    lastChoice = -1
    loop do
  ...
  def pbPlayerChooseCard(numCards)
    if @battle.openHand
      @sprites["helpwindow"].text = _INTL("选择一张卡或按Z查看对手。")
    else
      @sprites["helpwindow"].text = _INTL("选择卡片")
    end
    choice     = 0
    lastChoice = -1
    loop do
  ...
  def pbPlayerPlaceCard(cardIndex)
    @sprites["helpwindow"].text = _INTL("放置卡片")
    boardX = 0
    boardY = 0
    doRefresh = true
    loop do
...
#         attack += attacker.bonus(defender)
  ...
  def pbStartScreen(opponentName, minLevel, maxLevel, rules = nil, oppdeck = nil, prize = nil)
    raise _INTL("最低级别必须在0到9内。") if minLevel < 0 || minLevel > 9
    raise _INTL("最大级别必须在0到9内。") if maxLevel < 0 || maxLevel > 9
    raise _INTL("最大值不应小于最小值。") if maxLevel < minLevel
    ...
    if count < self.maxCards
      @scene.pbDisplayPaused(_INTL("你的卡片不够。"))
      @scene.pbEndScene
      return 0
    end
    ...
    if oppdeck.is_a?(Array) && oppdeck.length == self.maxCards   # Preset
      ...
      oppdeck.each do |species|
        ...
        if !species_data
          @scene.pbDisplayPaused(_INTL("对手有一张非法的卡片，“{1}”。", species))
          @scene.pbEndScene
          return 0
        end
    ...
    @scene.pbDisplay(_INTL("请选择首发……"))
    @scene.pbUpdateScore
    playerTurn = (rand(2) == 0)
    @scene.pbDisplay(_INTL("{1}是首发。", (playerTurn) ? @playerName : @opponentName))
    (@width * @height).times do |i|
      position = nil
      triadCard = nil
      cardIndex = 0
      if playerTurn
        ...
        # Opponent's turn
        @scene.pbDisplay(_INTL("{1}正在移动……", @opponentName))
        scores = []
    ...
    if playerCount == opponentCount
      @scene.pbDisplayPaused(_INTL("这场比赛是平局。"))
      result = 3
      if @trade == 1
        ...
        @scene.pbDisplayPaused(_INTL("保留了所有颜色的卡片。"))
      end
      ...
      if prize
        ...
        if species_data && $PokemonGlobal.triads.add(species_data.id)
          @scene.pbDisplayPaused(_INTL("赢得了{1}卡片。", species_data.name))
        end
      else
        case @trade
        when 0   # Gain 1 random card from opponent's deck
          ...
          if $PokemonGlobal.triads.add(card)
            ...
            @scene.pbDisplayPaused(_INTL("赢得了{1}卡片。", cardname))
          end
        when 1   # Keep only cards of your color
          ...
          @scene.pbDisplayPaused(_INTL("保留了你所有颜色的卡片。"))
        when 2   # Gain all opponent's cards
          originalOpponentCards.each { |crd| $PokemonGlobal.triads.add(crd) }
          @scene.pbDisplayPaused(_INTL("赢走了对手所有的卡片。"))
        end
      end
    else
      @scene.pbDisplayPaused(_INTL("{1}输给了{2}。", @playerName, @opponentName))
      result = 2
      case @trade
      when 0   # Lose 1 random card from your deck
        ...
        cardname = GameData::Species.get(card).name
        @scene.pbDisplayPaused(_INTL("对手赢走了{1}张卡片。", cardname))
      when 1   # Keep only cards of your color
        ...
        @scene.pbDisplayPaused(_INTL("保留了你所有颜色的卡片。", cardname))
      when 2   # Lose all your cards
        originalCards.each { |crd| $PokemonGlobal.triads.remove(crd) }
        @scene.pbDisplayPaused(_INTL("对手赢走了你所有的卡片。"))
      end
    end
    @scene.pbEndScene
    return result
  ...
  loop do
    ...
    if Input.trigger?(Input::BACK)
      ...
      quantity = pbMessageChooseNumber(
        _INTL("{1}卡是吗？您想要多少？", itemname), params
      )
      next if quantity <= 0
      price *= quantity
      next if !pbConfirmMessage(_INTL("购买{1}{2}需要${3}，确定吗？", itemname, quantity, price.to_s_formatted))
      if $player.money < price
        pbMessage(_INTL("您没有足够的钱。"))
        next
      end
      if !$PokemonGlobal.triads.can_add?(item, quantity)
        pbMessage(_INTL("已经没有位置存放卡片了。"))
        next
      end
      $PokemonGlobal.triads.add(item, quantity)
      $player.money -= price
      goldwindow.text = _INTL("金钱：\n{1}", pbGetGoldString)
      pbMessage(_INTL("给您！谢谢惠顾！") + "\\se[Mart buy item]")
    end
  end
  cmdwindow.dispose
  goldwindow.dispose
  ...
  until done
    loop do
      ...
      if Input.trigger?(Input::USE)
        ...
        if price == 0
          pbDisplayPaused(_INTL("{1}卡？哦不，我不能买那个。", itemname))
          break
        end
        cmdwindow.active = false
        cmdwindow.update
        if quantity > 1
          ...
          params.setCancelValue(0)
          quantity = pbMessageChooseNumber(
            _INTL("{1}卡？您想要卖多少？", itemname), params
          )
        end
        if quantity > 0
          price /= 4
          price *= quantity
          if pbConfirmMessage(_INTL("我可以支付您${1}，\n可以接受吗？", price.to_s_formatted))
            $player.money += price
            goldwindow.text = _INTL("金钱：\n{1}", pbGetGoldString)
            $PokemonGlobal.triads.remove(item, quantity)
            pbMessage(_INTL("你交出了{1}卡，获得了${2}。", itemname, price.to_s_formatted) + "\\se[Mart buy item]")
            commands = []
            $PokemonGlobal.triads.length.times do |i|
              item = $PokemonGlobal.triads[i]
              speciesname = GameData::Species.get(item[0]).name
