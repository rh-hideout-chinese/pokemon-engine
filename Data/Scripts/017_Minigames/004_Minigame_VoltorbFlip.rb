...
class VoltorbFlip
  ...
  def pbNewGame
    ...
    pbDrawShadowText(@sprites["text"].bitmap, 8, 88, 118, 26,
                     _INTL("获得的代币"), Color.new(60, 60, 60), Color.new(150, 190, 170), 1)
    # Draw current level
    ...
    if $player.coins >= Settings::MAX_COINS
      pbMessage(_INTL("已经获得了{1}枚代币，不能再获得代币了。", Settings::MAX_COINS.to_s_formatted))
      $player.coins = Settings::MAX_COINS   # As a precaution
      @quit = true
#    elsif !pbConfirmMessage(_INTL("要玩霹雳翻牌Lv. {1}吗？", @level)) && $player.coins < Settings::MAX_COINS
#      @quit = true
    else
        ...
        @squares.length.times do |i|
          if (@index[0] * 64) + 128 == @squares[i][0] && @index[1] * 64 == @squares[i][1] && @squares[i][3] == false
            ...
            if @squares[i][2] == 0
              ...
              pbMessage("\\me[Voltorb Flip game over]" + _INTL("哦不！一个代币都没有获得！") + "\\wtnp[80]")
              pbShowAndDispose
              @sprites["mark"].bitmap.clear
              if @level > 1
                ...
                if @level > newLevel
                  @level = newLevel
                  @level = 1 if @level < 1
                  pbMessage("\\se[Voltorb Flip level down]" + _INTL("游戏降低到Lv.{1}了!", @level.to_s))
                end
              end
              # Update level text
...
#        pbMessage(_INTL("找到了所有的x2和x3卡。"))
#        pbMessage(_INTL("这意味着找到了所有的代币，游戏现在结束了。"))
        pbMessage("\\se[Voltorb Flip gain coins]" + _INTL("{1}收到了{2}枚代币！", $player.name, @points.to_s_formatted))
        # Update level text
        @sprites["level"].bitmap.clear
        ...
        if @level < 8
          @level += 1
          pbMessage("\\se[Voltorb Flip level up]" + _INTL("游戏提升到Lv.{1}了！", @level.to_s))
          if @firstRound
#            pbMessage(_INTL("恭喜！"))
#            pbMessage(_INTL("可以在下场游戏收到更多代币！"))
            @firstRound = false
          end
        end
        pbDisposeSpriteHash(@sprites)
      ...
      elsif pbConfirmMessage(_INTL("现在退出会获得{1}枚代币，确定退出吗？",
                                   @points.to_s_formatted))
        pbMessage(_INTL("{1}收到了{2}枚代币！", $player.name, @points.to_s_formatted))
        old_coins = $player.coins
        $player.coins += @points
        $stats.coins_won += $player.coins - old_coins if $player.coins > old_coins
        @points = 0
...
def pbVoltorbFlip
  if !$bag.has?(:COINCASE)
    pbMessage(_INTL("除非拥有代币盒，否则无法游玩。"))
  elsif $player.coins == Settings::MAX_COINS
    pbMessage(_INTL("代币盒装满了！"))
  else
