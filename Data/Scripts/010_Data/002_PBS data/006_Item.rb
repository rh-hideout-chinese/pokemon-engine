module GameData
  class Item
    ...
    def self.editor_properties
      field_use_array = [_INTL("无法在此处使用")]
      self.schema["FieldUse"][2].each { |key, value| field_use_array[value] = key if !field_use_array[value] }
      battle_use_array = [_INTL("无法在对战中使用")]
      self.schema["BattleUse"][2].each { |key, value| battle_use_array[value] = key if !battle_use_array[value] }
      return [
        ["ID",                ReadOnlyProperty,                        _INTL("此道具的ID。 （如：XXX）")],
        ["Name",              ItemNameProperty,                        _INTL("此道具的名称。")],
        ["NamePlural",        ItemNameProperty,                        _INTL("此道具的复数名。")],
        ["PortionName",       ItemNameProperty,                        _INTL("一份此道具时显示的 名称。")],
        ["PortionNamePlural", ItemNameProperty,                        _INTL("多份此道具时显示的 名称。")],
        ["Pocket",            PocketProperty,                          _INTL("存放此道具的口袋。")],
        ["Price",             LimitProperty.new(Settings::MAX_MONEY),  _INTL("此道具的购价。")],
        ["SellPrice",         LimitProperty2.new(Settings::MAX_MONEY), _INTL("此道具的售价。如果 未填写，则为购价的 一半。")],
        ["BPPrice",           LimitProperty.new(Settings::MAX_BATTLE_POINTS), _INTL("此道具用对战点数 （BP）购买时的价格 。")],
        ["FieldUse",          EnumProperty.new(field_use_array),       _INTL("此道具在对战外使用 的方式。")],
        ["BattleUse",         EnumProperty.new(battle_use_array),      _INTL("此道具在对战时使用 的方式。")],
        ["Flags",             StringListProperty,                      _INTL("将道具分组的某些英 文单词/短语。")],
        ["Consumable",        BooleanProperty,                         _INTL("此道具使用后是否会 被消耗。")],
        ["ShowQuantity",      BooleanProperty,                         _INTL("是否在背包里显示此 道具的数量。")],
        ["Move",              MoveProperty,                            _INTL("使用此HM/TM/TR学会 的招式。")],
        ["Description",       StringProperty,                          _INTL("此道具的描述。")]
      ]
    end
