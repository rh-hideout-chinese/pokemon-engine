...
class Pokemon
  ...
  def level=(value)
    if value < 1 || value > GameData::GrowthRate.max_level
      raise ArgumentError.new(_INTL("等级编号（{1}）无效", value))
    end
  ...
  def status=(value)
    ...
    if !new_status
      raise ArgumentError, _INTL("试图将 {1} 设置为宝可梦的状态", value.class.name)
    end
  ...
  def mail=(mail)
    if !mail.nil? && !mail.is_a?(Mail)
      raise ArgumentError, _INTL("给定值{1}无效", mail.inspect)
    end
    @mail = mail
  end
  ...
  def changeHappiness(method)
    ...
    when "revivalherb"
      ...
      raise _INTL("未知的好感改变方法: {1}", method.to_s)
    end
    if gain > 0
