...
def pbPurify(pkmn, scene)
  ...
  if pkmn.saved_exp
    ...
    curlevel = pkmn.level
    if newexp != pkmn.exp
      scene.pbDisplay(_INTL("{1}重新获得了{2}经验！", pkmn.name, newexp - pkmn.exp))
    end
  ...
  if $PokemonSystem.givenicknames == 0 &&
     scene.pbConfirm(_INTL("要给{1}起昵称吗？", pkmn.speciesName))
    newname = pbEnterPokemonName(_INTL("{1}的昵称是？", pkmn.speciesName),
...
def pbRelicStone
  if $player.party.none? { |pkmn| pkmn.purifiable? }
    pbMessage(_INTL("没有可以净化的宝可梦。"))
    return
  end
  pbMessage(_INTL("也许有宝可梦能打开心门！"))
...
class Battle
  ...
  def pbCanUseItemOnPokemon?(item, pkmn, battler, scene, showMessages = true)
    ...
    if ret && pkmn.hyper_mode && ![:JOYSCENT, :EXCITESCENT, :VIVIDSCENT].include?(item)
      scene.pbDisplay(_INTL("不能对那个宝可梦使用。"))
      return false
    end
    return ret
  end
...
class Battle::Battler
  ...
  def pbHyperMode
    ...
    if @battle.pbRandom(p.heart_gauge) <= p.max_gauge_size / 4
      p.hyper_mode = true
      @battle.pbDisplay(_INTL("{1}逐渐变得狂暴！{1}进入了暴走状态！", self.pbThis))
    end
  end
...
def pbRaiseHappinessAndReduceHeart(pkmn, scene, multiplier, show_fail_message = true)
  if !pkmn.shadowPokemon? || (pkmn.happiness == 255 && pkmn.heart_gauge == 0)
    scene.pbDisplay(_INTL("即便使用也无效果哦。")) if show_fail_message
    return false
  end
  ...
  if pkmn.heart_gauge == old_gauge
    scene.pbDisplay(_INTL("{1}变得友好了。", pkmn.name))
  elsif pkmn.happiness == old_happiness
    scene.pbDisplay(_INTL("{1}十分喜欢你！\n心门打开了一点。", pkmn.name))
    pkmn.check_ready_to_purify
  else
    scene.pbDisplay(_INTL("{1}变得友好了。\n心门打开了一点。", pkmn.name))
    pkmn.check_ready_to_purify
  end
  return true
end
...
ItemHandlers::UseOnPokemon.add(:JOYSCENT, proc { |item, qty, pkmn, scene|
  ret = false
  if pkmn.hyper_mode
    scene.pbDisplay(_INTL("在{2}的呼唤下，{1}清醒过来了！", pkmn.name, GameData::Item.get(item).name))
    pkmn.hyper_mode = false
    ret = true
  end
...
ItemHandlers::BattleUseOnPokemon.add(:JOYSCENT, proc { |item, pokemon, battler, choices, scene|
  if pokemon.hyper_mode
    ...
    scene.pbDisplay(_INTL("在{2}的呼唤下，{1}清醒过来了！",
...
class Battle::Move::AllBattlersLoseHalfHPUserSkipsNextTurn < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("所有宝可梦的HP减半！"))
...
class Battle::Move::RemoveAllScreensAndSafeguard < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("打破了所有的阻碍！"))
  end
end
