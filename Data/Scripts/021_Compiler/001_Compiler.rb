...
module FileLineData
  ...
  def self.linereport
    if @section
      if @key.nil?
        return _INTL("文件{1}，第{2}部分\n{3}", @file, @section, @value) + "\n\n"
      else
        return _INTL("文件{1}，第{2}部分，键{3}\n{4}", @file, @section, @key, @value) + "\n\n"
      end
    else
...
module Compiler
      ...
      if !line[/^\#/] && !line[/^\s*$/]
        ...
        if line[/^\s*\[\s*(.*)\s*\]\s*$/]   # Of the format: [something]
          ...
          if sectionname.nil?
            ...
            raise _INTL("文件开头需要一个部分。（如果文件未以UTF-8格式保存，也可能会发生此错误。）\n{1}", FileLineData.linereport)
          end
          if !line[/^\s*(\w+)\s*=\s*(.*)$/]
            FileLineData.setSection(sectionname, nil, line)
            raise _INTL("错误的行语法（预期语法如XXX=YYY）\n{1}", FileLineData.linereport)
          end
          r1 = $~[1]
          r2 = $~[2]
    ...
    f.each_line do |line|
      ...
      if !line[/^\#/] && !line[/^\s*$/]
        if line[/^\s*\[\s*(.+?)\s*\]\s*$/]
          ...
          if sectionname.nil?
            raise _INTL("文件开头需要一个部分。（第{1}行）每个部分以 “[此部分的名称]” 开头。", lineno)
          end
  ...
  def csvBoolean!(str, _line = -1)
    ...
    raise _INTL("{1} 不是布尔值（true、false、1、0）\n{2}", field, FileLineData.linereport)
  end
  ...
  def csvFloat!(str, _line = -1)
    ret = csvfield!(str)
    return Float(ret) rescue raise _INTL("{1} 不是数字\n{2}", ret, FileLineData.linereport)
  end
  ...
  def cast_csv_value(value, schema, enumer = nil)
    ...
    when "v"   # Positive integer
      ...
      if value.to_i == 0
        raise _INTL("“{1}”必须大于 0\n{2}", value, FileLineData.linereport)
      end
      return value.to_i
    when "x"   # Hexadecimal number
      if !value[/^[A-F0-9]+$/i]
        raise _INTL("“{1}”不是十六进制数\n{2}", value, FileLineData.linereport)
      end
      return value.hex
    ...
    when Module
      begin
        if nil_or_empty?(ret) || !enumer.const_defined?(ret)
          raise _INTL("{2} 中的值 {1} 未定义\n{3}", ret, enumer.name, FileLineData.linereport)
        end
      rescue NameError
        raise _INTL("{2} 中的值 {1} 不正确\n{3}", ret, enumer.name, FileLineData.linereport)
      end
  ...
  def parseSpecies(species)
    ...
    if !spec
      raise _INTL("未定义的物种常量名称：{1}\n请确保此物种已在 PBS/pokemon.txt 中定义。\n{2}", species, FileLineData.linereport)
    end
    return spec.id
  end
  ...
  def write_pbs_file_message_start(filename)
    # The `` around the file's name turns it cyan
    Console.echo_li(_INTL("正在生成 PBS 文件 “{1}”", filename.split("/").last))
  end
  ...
  def compile_all(mustCompile)
    Console.echo_h1(_INTL("检查游戏数据"))
    if !mustCompile
      Console.echoln_li(_INTL("游戏数据未编译"))
      echoln ""
      return
    end
    FileLineData.clear
    ...
    Console.echo_done(true)
    Console.echoln_li_done(_INTL("成功编译所有游戏数据"))
  end
