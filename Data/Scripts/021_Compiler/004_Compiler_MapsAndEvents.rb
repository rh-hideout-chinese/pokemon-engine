module Compiler
  ...
  def import_new_maps
    ...
    if imported
      ...
      pbMessage(_INTL("复制到Data文件夹的{1}个地图已成功导入。", count))
    end
    return imported
  end
          ...
          if params[0][/SellItem\s*\(\s*(\w+)\s*\,\s*(\d+)\s*\)/]
            ...
            if GameData::Item.exists?(itemname)
              ...
              if cost == 0
                ...
                push_text(newEvents, _INTL("你的钱不够……"), oldIndent + 1)
              else
                ...
                push_text(newEvents, _INTL("\\G你的钱不够……"), oldIndent + 2)
                push_branch_end(newEvents, oldIndent + 2)
                push_else(newEvents, oldIndent + 1)
                push_text(newEvents, _INTL("\\G您没有足够的钱。"), oldIndent + 1)
              end
...
#            end
  ...
  def compile_trainer_events(_mustcompile)
    ...
    change_record = []
    Console.echo_li(_INTL("正在处理{1}个地图……", mapData.mapinfos.keys.length))
    idx = 0
