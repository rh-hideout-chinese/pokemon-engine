...
module Translator
  ...
  def extract_text(language_name = "default", core_text = false, separate_map_files = false)
    ...
    if core_text
      ...
      if !default_messages || default_messages.length == 0
        pbMessageDisplay(msg_window, _INTL("默认核心信息文件“messages_core.dat”未找到。"))
      ...
      if !default_messages || default_messages.length == 0
        pbMessageDisplay(msg_window, _INTL("默认游戏信息文件“messages_game.dat”未找到。"))
        pbDisposeMessageWindow(msg_window)
        return
      end
    end
    ...
    if Dir.safe?(dir_name)
      ...
      if has_files && !pbConfirmMessageSerious(_INTL("替换文件夹‘{1}’中的所有文本文件?", dir_name))
        pbDisposeMessageWindow(msg_window)
        return
      end
    ...
    # Extract the text
    pbMessageDisplay(msg_window, "\\ts[]" + _INTL("提取文本中，请稍等。") + "\\wtnp[0]")
    ...
    pbMessageDisplay(msg_window, _INTL("若要进行翻译，请翻译文件中每两行的第二行。") + "\1")
    pbMessageDisplay(msg_window, _INTL("在翻译完成后，请选择调试菜单中的“编译翻译文本”。"))
    pbDisposeMessageWindow(msg_window)
  end
  ...
  def compile_text(dir_name, dat_filename)
    msg_window = pbCreateMessageWindow
    pbMessageDisplay(msg_window, "\\ts[]" + _INTL("编译文本中，请稍等。") + "\\wtnp[0]")
    ...
    pbMessageDisplay(msg_window,
       _INTL("文件夹“Text_{1}”中的文本文件已编译为“Data/messages_{2}.dat”文件。", dir_name, dat_filename))
    pbMessageDisplay(msg_window, _INTL("你可能需要重启游戏才能应用修改。"))
    pbDisposeMessageWindow(msg_window)
  end
  ...
  def compile_text_from_file(text_file, all_text)
    begin
      ...
      raise _INTL("无法找到或打开‘{1}’。", text_file)
    end
    begin
      Compiler.pbEachSection(file) do |contents, section_name|
        ...
        raise _INTL("{1}部分的名称无效", section_name) if section_id < 0
        ...
        if contents[0][/^\d+$/]   # If first line is a number, text is stored in an array
          ...
          item_length = 3
          if is_map
            raise _INTL("{1}部分不能是有序列表（因为其第一行是数字，所以此部分被识别为有序列表）。", section_name)
          end
          if contents.length % 3 != 0
            raise _INTL("{1}部分的行数不能被3整除（因为其第一行是数字，所以此部分被识别为有序列表）。", section_name)
          end
        else   # Text is stored in a hash
          text_hash = {}
          item_length = 2
          if contents.length.odd?
            raise _INTL("{1}部分的行数为奇数（因为其第一行不是数字，所以此部分被识别为哈希表单）。", section_name)
          end
        end
        ...
        loop do
          if item_length == 3
            if !contents[i][/^\d+$/]
              raise _INTL("在{1}部分预期得到一个数字，但是却得到了{2}", section_name, contents[i])
            end
            key = contents[i].to_i
            i += 1
          else
